...
const SITES: Record<string, Site> = {
  web: {
    ...
    enter: true,
    routes: () =>
      readdirSync(join(root, "sites/web/fixtures"))
...
function send(method: string, params = {}, deadline = 15000): Promise<any> {
  return new Promise((resolve, reject) => {
    id += 1
    const key = id
    const timer = setTimeout(() => {
      pending.delete(key)
      reject(new Error(`chrome did not answer ${method} within ${deadline}ms`))
    }, deadline)
