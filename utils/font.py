...
PIXEL = 125
UPEM = 1000
OUT = os.path.join(ROOT, "pkgs", "rs", "mrlyfont", "assets")
...
def build(ttf_path):
    glyphs = mp.read(handle(), "fonts/glyphs")
    descenders = set(mp.read(handle(), "fonts/descenders"))
    ...
    fb.font["head"].created = timestampSinceEpoch(CREATED.timestamp())
    fb.font["head"].modified = fb.font["head"].created
...
def manifest():
    ...
    with open(path, "w", encoding="utf-8") as f:
        f.write(mp.read(handle(), "fonts/json"))
        f.write("\n")
    print(f"Saved: {path}")
