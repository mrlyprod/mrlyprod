...
function reading(name, cells) {
  const named = name.match(/^(bang_[^.]+)\.(\w+)\.(\w+)$/);
  if (!named) return null;
  try {
    const bang = JSON.parse(m.name_parse(named[1]));
    return JSON.parse(m.ledger_row(bang.code, bang.dim, bang.base, named[2], named[3], TERMS, cells));
  } catch {
    return null;
  }
}
  ...
  const made = useMemo(() => {
    try {
      if (row.d === 2) {
        ...
        return { title: `side ${row.number}, level ${level}`, art: <Grid grid={m.two_grid(row.code, row.number, level, 0, row.q)} on={ink.blue} role="img" aria-label={`The design ${row.code}, side ${row.number}, level ${level}`} /> };
      }
      if (row.d === 3) {
        const level = m.level_cap(row.number, 3, 8000);
        return { title: `side ${row.number}, level ${level}, isometric`, art: <Markup svg={m.hex_svg(row.code, row.number, level, row.q, 'iso', 4)} role="img" aria-label={`The design ${row.code}, side ${row.number}, level ${level}, isometric`} /> };
      }
      if (row.d === 1) {
        const level = m.level_cap(row.number, 1, 729);
        const cells = m.ledger_profile(row.code, 1, row.q, row.number, level);
        return { title: `side ${row.number}, level ${level}, the strip`, art: <Grid grid={{ width: cells.length, height: 1, types: Uint8Array.from(cells) }} on={ink.blue} style={{ height: 48 }} role="img" aria-label={`The design ${row.code}, side ${row.number}, level ${level}, the strip`} /> };
      }
      ...
      return { title: `side ${row.number}, level ${level}, the diagonal profile`, art: <Sketch className="bars" draw={draw} deps={[counts]} role="img" aria-label={`The design ${row.code}, side ${row.number}, level ${level}, the diagonal profile`} /> };
    } catch (error) {
...
function View({ row, view, dig, blend }) {
  ...
  const named = (what) => `The terms of ${row.name}, ${what}`;
  if (view === 'digits') return <Digits terms={row.terms} start={row.start} base={dig} label={label} role="img" aria-label={named('a heatmap of the digits of every term')} />;
  if (view === 'steps') return <Staircase terms={row.terms} start={row.start} label={label} role="img" aria-label={named('as a step function')} />;
  if (view === 'sums') return <Staircase terms={row.terms} start={row.start} sums label={label} role="img" aria-label={named('as partial sums')} />;
  if (view === 'ratios') return blend ? <Ratios values={blend.ratios} start={row.start} label={`${label}, each term against the one before`} role="img" aria-label={named('each term against the one before')} /> : null;
  if (view === 'differences') return blend ? <Differences rows={blend.differences} label={`${label}, each row the differences of the row above`} role="img" aria-label={named('the difference triangle')} /> : null;
  return <Pins terms={row.terms} start={row.start} label={label} role="img" aria-label={named('every term standing on its index')} />;
}
...
function App() {
  ...
  const controls = (
    <>
      <Group name="Search">
        <label>search <input type="text" style={{ flex: 1, minWidth: 0 }} value={pick.q} placeholder="6, 42, 306 or carpet or A000567 or 23" onChange={(e) => { s.drop(); set({ q: e.target.value, page: '' }); }} /></label>
