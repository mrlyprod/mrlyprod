...
function App() {
  ...
  const controls = (
    <>
      ...
      <Group name="Shell">
        ...
        <Slider label="threshold" value={mark} min={0} max={cap} step={1} onChange={(v) => setThreshold(v >= cap ? null : v)} />
      ...
      <Group name="Camera">
        ...
        <span className="tabs" role="group" aria-label="Look from">
  ...
  return (
    <Page crumb="volume" title="A stack of scales becomes a solid you can cut" controls={controls}
      ...
      <div className="arena" style={{ gridTemplateColumns: '2fr 1fr' }}>
        <div className="panel">
          ...
          <Stage role="img" aria-label="The solid" deps={[view, q.opacity, q.camera]} onStage={(st) => {
            live.current = st;
        ...
        <div className="panel">
          ...
          {view.cut ? <Pixels data={view.cut} role="img" aria-label="The cut" /> : <canvas className="sheet" role="img" aria-label="The cut, none taken" />}
        </div>
      </div>
      <Stats>
