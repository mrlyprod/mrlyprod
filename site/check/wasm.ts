...
const checks: [string, unknown, unknown][] = [
  ...
  ['chladni_run 7 3 3 living soup', `${sum(chladniLive.types)},${chladniRead.peak_ring},${chladniRead.wavelength}`, '3274,1,128'],
  ...
  ['random_code 3 2 seed 7', m.random_code(3, 2, 7), '84'],
  ['random_codes 3 2 seed 7', m.random_codes(3, 2, 7, 3).join(','), '84,17,216'],
  ['random_between seed 7', Array.from(m.random_between(7, [0, 0, 1], [3, 1800, 36])).join(','), '2,936,23'],
  ...
  ['ledger_search octagonal', `${octagon.total} ${octagon.rows[0].name} ${octagon.rows[0].oeis} ${octagon.rows[0].shift} ${octagon.rows[0].tag} ${octagon.rows[0].closed}`, '1 sequence_dim=2_code=7_measure=fills_axis=side A000567 0 Proved 3k^2 - 2k'],
  ...
  ['ledger_row void side', `${hollow.name} ${hollow.terms.join(',')} ${hollow.closed} ${hollow.number}`, 'sequence_dim=2_code=9_measure=voids_axis=side 4,12,24 2k^2 - 2k 3'],
  ...
  ['census_writers 16 heads', `${writes16.rows} ${writes16.shown[0].name} ${writes16.shown[0].closed} ${writes16.shown[0].index}`, '633 sequence_dim=1_code=1_measure=fills_axis=level 2^level 3'],
...
checks.push(
  ...
  ['blend_series sponge row', `${plotSeries.name} ${plotSeries.oeis} ${plotSeries.closed}`, 'sequence_dim=3_code=23_measure=surface_axis=level A332705 a(level) = 28 a(level-1) - 160 a(level-2)'],
