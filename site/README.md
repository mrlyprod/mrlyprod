...
- Routes: `/`, `/demos/`, `/demos/<name>/`, `/papers/`, `/papers/<slug>/`, `/research/`, `/research/<name>/`, `/research/figures/<file>`, `/blog/`, `/blog/<slug>/`, `/menu/`, `/about/`, `/contact/`, `/donate/`, `/cart/`, `/git/...`, `/raw/...` and `/404.html`, beside `sitemap.xml`, `robots.txt`, `llms.txt`, `manifest.webmanifest` and the icons the builder draws from the logo.
...
- The figures a research note draws are one hidden route: it publishes `../research/figures/` under `/research/figures/`, so a note's `figures/x.png` link resolves to that page and the copies live and die with the route.
...
- [spirograph](demos/spirograph/) - A design is the wheel and its cells are the holes, a pencil in every one, rolled without slipping on a line, a circle or a polygon: every pencil draws a trochoid, and on a circle two pencils draw one curve exactly when a rotation of a full turn over the wheel's reduced radius carries one seat onto the other.
