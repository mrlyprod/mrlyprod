...
- `walk bytes forget digest short escape page`: the small helpers the site would write twice. `forget()` drops the byte cache, which a watcher calls before it rescans.
