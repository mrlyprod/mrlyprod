...
- `tokens.css` resolves the roles `--ground --bg --panel --deep --line --fg --dim --accent --on-accent` per theme, plus the figure switches `--show-dark` / `--show-light`.
...
- `chrome.js`: vanilla ESM, runs on load; the scroll spot, drawers, theme, font, tint, saver, cart, contents highlight, footer mark, tree reveal, lazy tree; exports `wire()` for pages that render later.
