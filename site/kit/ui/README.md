...
- `palette.css` names black, white and thirteen hues (`--red --orange --yellow --green --mint --teal --cyan --blue --indigo --purple --pink --brown --gray`), one flat value each, no shades.
...
- `palette.css`: generated, one `:root` block, the fifteen colors and nothing else.
- `palette.js`: generated, the same palette for scripts.
- `tokens.css`: loaded after `palette.css`; the roles light on `:root` and dark twice, `--art --scrim --mix`, type (system stacks ending in Noto Symbols 2 and Noto Color Emoji as fallbacks), `--face` per `data-font`, space, shape, frame, motion.
...
- `theme.js`: the two role sets, `dark` and `light`, over `palette.js` for scripts that paint without the DOM; `savers/frame.js` and the demos' `mrly.js` read it. Listed under `kit.files` like every file a kit script imports.
...
- `fonts/`: `fonts.css` and the vendored faces, all OFL with their licences beside them: Noto Sans, Noto Serif and Noto Sans Mono (variable 400-700, Latin), MrlyFont from `crates/mrlyfont`, Noto Sans Symbols 2, and Noto Color Emoji in ten unicode-range shards. A site lists them under an `assets` bundle with `hash: true`, the builder rewrites each `url()` in the css to the hashed name, and the head links `site.asset("fonts/fonts.css")`; a face is fetched only when a page needs it. `symbols.woff2` is cut from the master at `files/fonts/symbols.ttf` down to the glyphs the built pages print, with `bun run symbols`, which rewrites its `src` and `unicode-range` here; `kit/ui/fonts.test.ts` fails if a page prints a symbol the cut face lacks.
