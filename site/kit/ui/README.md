...
- `chrome.jsx`: `Shell({ route, title, lead, tree, current, contents, controls, wide, children })`, `Header()`, `Dock({ route })`, `Footer()`, `Wordmark({ className })`, `Glyph({ text, className, label })`, `Tree({ nodes, current })`, `Grid({ nodes })`, `Menu({ tree })`, `Contents({ items, current })`, `Controls({ children })`, `Settings`.
