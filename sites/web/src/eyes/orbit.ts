...
import rigsData from "../gen/rigs.json"
import type { Call, Rigs } from "../types"

export type Orbit = { yaw: number; pitch: number; dist: number; pan: [number, number]; ortho: boolean }

const baked = rigsData as Rigs

const TAU = Math.PI * 2
const TURN = baked.turn
const NONE: Orbit = { yaw: 0, pitch: 0, dist: 0, pan: [0, 0], ortho: false }
const RIGGED = new Set(Object.keys(baked.rigs))
...
export function orbit(app: string): Orbit {
  ...
  if (rig === undefined) {
    const rest = baked.rigs[app] ?? NONE
    rig = { ...rest, pan: [rest.pan[0] ?? 0, rest.pan[1] ?? 0] }
    rigs.set(app, rig)
  }
  return rig
}
