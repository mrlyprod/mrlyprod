    ...
    <Stack>
      <Box>
        <Choice
          ...
          value={s.category}
          onChange={value => send(call("emojis.set", { key: "category", value }))}
        />
      ...
      <Section label={s.category}>
        <Grid cols={8}>
          {s.grid.map(glyph => (
            <Button key={glyph} onClick={() => send(call("emojis.keep", { value: glyph }))}>
      ...
      <Section label="library">
        ...
        <Grid cols={8}>
          {s.library.map(glyph => (
            <Button key={glyph} onClick={() => send(call("emojis.drop", { value: glyph }))}>
              <Emoji glyph={glyph} label={glyph} />
            </Button>
          ))}
        </Grid>
