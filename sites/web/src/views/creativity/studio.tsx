...
export function Studio({ state }: { state: unknown }) {
  ...
  return (
    <Stack>
          ...
          {s.keys.map((key, i) =>
            key === null ? (
              ...
              <Button
                key={key.midi}
                active={key.held}
                onPress={() => send(call("studio.press", { midi: key.midi }))}
                onLift={() => send(call("studio.lift", { midi: key.midi }))}
              >
                {key.name}
              </Button>
            ),
      ...
      <Section label="held">
        ...
        <Button disabled={names.length === 0} onClick={() => send(call("studio.silence"))}>
          silence
        </Button>
      </Section>
    </Stack>
