...
import { Store, stops, upto } from "./journal"
...
import type { Call, Observation, Palette, Registry, Send, Shaders } from "./types"
...
const KERNEL = new URL("../../../pkgs/js/web/pkg/mrlyweb_bg.wasm", import.meta.url)

export async function boot(): Promise<Face> {
  await load(fetch(KERNEL))
  let handle = wake("arcade")
  ...
  const replay = (calls: Call[], head?: string) => {
    if (store.journal.snapshot !== undefined) {
      call(handle, { verb: "sys.thaw", args: { state: store.journal.snapshot } })
    }
    for (const kept of calls) call(handle, kept)
    if (head !== undefined && observe(handle).route?.app !== head) {
      call(handle, { verb: "nav.open", args: { app: head } })
    }
  ...
  const send: Send = (made, beat = false) => {
    ...
    const looking = at !== null && (made.verb === "nav.open" || quiet)
    if (at !== null && !looking) {
      const kept = upto(store.journal, at)
      const head = observe(handle).route?.app
      if (head !== undefined) kept.push({ verb: "nav.open", args: { app: head }, now: Date.now() })
      store.fork(kept)
      at = null
      toast("time forked here")
    }
    const shot = made.verb === "sys.shot" ? shoot(made) : made
    const stamped = { ...shot, now: Date.now() }
    if (!looking) store.record(stamped, beat)
    ...
    const span = stops(store.journal)
    setTime(at ?? span, span)
  }
  ...
  const seek = (to: number) => {
    const span = stops(store.journal)
    const landed = Math.min(Math.max(to, 0), span)
    handle.free()
    handle = wake("arcade")
    replay(upto(store.journal, landed))
    at = landed >= span ? null : landed
    setTime(at ?? span, span)
    const obs = observe(handle)
    publish(obs)
    atPath = path(obs)
  ...
  routes.on("journal.seek", made => {
    const to = (made.args as { to?: unknown }).to
    if (typeof to === "number" && Number.isInteger(to)) seek(to)
  })
