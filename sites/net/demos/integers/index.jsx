    ...
    <Page crumb="integers" title="The integers this machine writes, and the ones it misses"
      ...
      <div className="arena">
        <div className="panel">
          ...
          <Sketch draw={field} deps={[look, value]} aria-label="The field, one cell an integer, click to read one" onPointerDown={onField} />
          <div className="stats">
          ...
          <Sketch className="bars" style={{ height: 62 }} draw={split} deps={[look, state.depth]} role="img" aria-label="The split of the window into missed, written once and written by many" />
        </div>
        <div className="panel">
              ...
              <tbody>
                {found.shown.length ? found.shown.map((row, i) => (
                  <tr key={`${i}:${row.name}`}>
                    ...
                    <td><Terms terms={row.head} tight /></td>
                  </tr>
      ...
      <div className="arena">
        <div className="panel">
          ...
          <Sketch className="bars" style={{ height: 210 }} draw={champions} deps={[champs, value]} aria-label="The champions, the integers the most rows write" onPointerDown={onChampions} />
          <h2>The first misses <span>click one to read it</span></h2>
          <Terms terms={misses} marks={misses.map((n) => n === value)} onPick={(n) => choose(Number(n))} empty="no integer of the window is missed" tight />
        </div>
        <div className="panel">
