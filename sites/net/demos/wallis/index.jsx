...
const gauge = (walk, limit, level) => (canvas) => {
  ...
  line(b, [at(level - 1)], ink.gold, { dots: 3.5 });
  tag(b, walk[level - 1].toFixed(9), ink.gold, 'right');
};
...
function App() {
  ...
  return (
    <Page crumb="wallis" title="The Wallis sieve buys area by growing its letters" controls={controls}
      ...
      {q.dim === 2 && data.grid && <Grid grid={data.grid} on={ink.gold} role="img" aria-label="The plane sieve" />}
