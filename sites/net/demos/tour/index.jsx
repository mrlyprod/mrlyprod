...
const CARDS = [
  ...
  },
  {
    ...
    build: (order) => {
      ...
      return {
        art: <Sketch className="" role="img" aria-label="The Farey stack" style={{ width: '100%', height: 200 }} draw={(canvas) => stackOf(canvas, order)} deps={[order]} />,
...
function Card({ spec, index, value, onChange, cardRef }) {
  ...
  return (
    <section className={`panel tour${spec.wide ? ' wide' : ''}`} ref={cardRef} aria-label={spec.title}>
      <div className="pic" role="img" aria-label={spec.title}>{made.art}</div>
      <div className="body">
