...
below has been recomputed in the lab - the millennium survey, the porous-flow
runs, the spectral-gap scripts, the level-spacings study and the
complex-dimensions study.
...
arithmetic under the reasons can. The survey script rebuilds the two
...
result, tagged claim by claim, lives on the primes page; the meter is the
Franel script beside it.
...
of the fresh runs, not the old note. Four independent routes now agree -
three lab scripts with different geometry constructions and different
solvers, all six conductances identical to ten decimal places - with a
fourth adversarial re-derivation during verification. Second, two clauses of the original claim did not
...
The spectral work is written up on the complexity page; the gap scripts live
beside it in the lab.

## Two supporting threads

Both belong to the Riemann row's clean-negative half and its structural
counterpart. The spacings story is written up on the complexity page; the
lattice story on the dimensions page.
