...
- Nine crates, one chain: `mrlycore` <- `mrlynum` <- `mrlymath` <- `mrlylab` <- `mrlydemo`, with `mrlyfig` on the laboratory, `mrlymusic` and `mrlyfont` on the substrate and `mrlygame` on both.
...
- `sites/kit/` the site kit: `ssg/` the builder, `git/` the code viewer, `ui/` the design kit every page wears; public but unpublished, fetched by sha.
