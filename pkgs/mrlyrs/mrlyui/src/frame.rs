...
impl Frame {
    ...
    pub fn to_json(&self) -> Json {
            ...
            .map(|layer| match layer {
                Layer::Tiles { ids, set } => {
                    ...
                    let grid: Vec<Vec<u8>> = (0..rows)
                        ...
                        .collect();
                    let colors: Vec<Vec<u8>> = set
                        .tiles
                        .iter()
                        .map(|t| representative(t).to_vec())
                        .collect();
                    json!({
                        "type": "tiles",
                        "rows": rows,
                    ...
                    let items: Vec<Json> = sprites
                        ...
                        .map(|s| {
                            json!({
                                ...
                                "w": s.cell.width(),
                                "h": s.cell.height(),
                                "color": representative(&s.cell).to_vec(),
                            })
                        })
                        .collect();
        ...
        let mut out = json!({
            "width": self.width,
            "height": self.height,
            "background": self.background.to_vec(),
            "layers": layers,
        });
        if !self.hud.empty() {
            out["hud"] = json!({
                "prompt": self.hud.prompt.clone(),
                "options": self.hud.options.clone(),
            });
        }
        out
    }
...
pub fn sprite_fact(cell: &Cell2d) -> Json {
    let (w, h) = (cell.width(), cell.height());
    let pixels: Vec<[u8; 4]> = (0..w * h).map(|i| cell.cell.color_at(i)).collect();
...
mod tests {
    ...
    fn image_indexes_a_palette() {
        ...
        assert_eq!(frame.fact(), image.to_json());
    }
    #[test]
