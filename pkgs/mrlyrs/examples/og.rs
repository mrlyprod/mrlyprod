...
fn main() {
    ...
    for app in mrly::net::registry::catalogue() {
        ...
        let png = mrly::ui::card::card_png(&m.route, &m.title).unwrap();
        fs::write(format!("{outdir}/{}.png", m.route), png).unwrap();
        count += 1;
    }
    let png = mrly::ui::card::card_png("home", "mrlyprod").unwrap();
    ...
    if let Ok(entries) = fs::read_dir("cdn/pages") {
        ...
        for slug in slugs {
            ...
            let title = md
                ...
                .unwrap_or("MrlyProd")
                .to_string();
            let png = mrly::ui::card::card_png(&format!("pages/{slug}"), &title).unwrap();
