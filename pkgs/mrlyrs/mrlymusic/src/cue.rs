...
mod tests {
    ...
    fn cues_are_notes_without_wave() {
        for name in ["blip", "good", "bad", "win", "lose"] {
            ...
            assert!(cue["freq"].as_i64().unwrap() > 0);
            assert!(cue["ms"].as_i64().unwrap() >= 90);
            assert!(cue["gain"].as_i64().unwrap() > 0);
            assert!(cue.get("wave").is_none());
        }
    }
    #[test]
    fn cues_land_their_offsets() {
        assert_eq!(payload("blip")["freq"], json!(391_995));
    ...
    fn gains_are_centi_percent() {
        assert_eq!(payload("blip")["gain"], json!(25));
        assert_eq!(payload("win")["gain"], json!(30));
    }
}
