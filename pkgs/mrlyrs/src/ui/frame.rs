...
use crate::core::tile::{Group, Tile};
use crate::math::two::tile as tile2d;
use crate::math::two::{designs, Cell2d};
...
pub fn board(dark: bool) -> [u8; 4] {
    let c = if dark {
        crate::core::colors::BOARD_DARK
    } else {
        crate::core::colors::BOARD_LIGHT
    };
    [c.r, c.g, c.b, c.a]
}
...
pub fn ink(dark: bool) -> [u8; 4] {
    let c = if dark {
        crate::core::colors::WHITE
    } else {
        crate::core::colors::BLACK
    };
    [c.r, c.g, c.b, c.a]
}
...
pub fn work_cell(tile: &Tile, paint: &Paint, k: usize, bg: [u8; 4]) -> Cell2d {
    ...
    if crate::core::paint::coat(&mut cell.cell, paint, None).is_err() {
        return solid_tile(k, solid);
    }
    let (w, h) = (cell.width(), cell.height());
    let mask = sample_types(&cell, k);
...
mod tests {
    ...
    fn work_cell_is_deterministic() {
        use crate::core::paint::{Edition, Ink, Paint, Scheme, Target};
        let bg = [0, 0, 0, 0];
