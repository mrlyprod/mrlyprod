...
fn prepare(seed: &Cell2d, config: &Config) -> Result<Cell2d> {
...
mod tests {
    ...
    use crate::life::{blinker, moore, Boundary, Counts, Source};
    ...
    fn blinker_is_a_loop_of_two() {
        let life = animate(&blinker(), &conway(moore().unwrap())).unwrap();
        assert_eq!(life.fate, Fate::Loop);
    ...
    fn block_is_alive_still_life() {
        ...
        let life = animate(&Cell2d::new(t).unwrap(), &conway(moore().unwrap())).unwrap();
