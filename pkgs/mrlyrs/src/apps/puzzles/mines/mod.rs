...
impl Mines {
    ...
    fn tileset(&self) -> TileSet {
        ...
        if digits {
            bake(&mut mine, "X", k, crate::ui::frame::board(self.dark));
        }
        tiles.push(mine);
        TileSet::new(k, tiles)
    }
...
mod tests {
    ...
    fn digits_skin_bakes_counts_into_the_canvas() {
        ...
        let board = crate::ui::frame::board(m.dark);
    ...
    fn moves_ring_the_right_cues() {
        ...
        assert_eq!(out.effects[0].data, crate::music::cue::payload("blip"));
        send(&mut m, "mines.flag", json!({ "cell": 0 }));
        let mine_cell = (0..m.size()).find(|&i| m.mine[i]).unwrap();
        let out = send(&mut m, "mines.reveal", json!({ "cell": mine_cell }));
        assert_eq!(out.effects[0].data, crate::music::cue::payload("lose"));
    }
    #[test]
