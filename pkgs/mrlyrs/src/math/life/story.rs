...
impl Story {
    ...
    pub fn add(&mut self, seed: &Cell2d, config: &Config) -> Result<Cell2d> {
        ...
        let last = life
            .last()
            .cloned()
            .ok_or_else(|| crate::core::MrlyError::Value("chapter produced no grids.".into()))?;
    ...
    pub fn pivot(&mut self, length: usize) -> Result<Cell2d> {
        let chapter = self
            .chapters
            .last_mut()
            .ok_or_else(|| crate::core::MrlyError::Value("no chapter to pivot.".into()))?;
        chapter.truncate(length);
        chapter
            .life
            .last()
            .cloned()
            .ok_or_else(|| crate::core::MrlyError::Value("chapter empty after pivot.".into()))
    }
    ...
    pub fn fate(&self) -> Result<Fate> {
        self.chapters
            .last()
            .map(|c| c.life.fate)
            .ok_or_else(|| crate::core::MrlyError::Value("empty story.".into()))
    }
}
...
mod tests {
    ...
    use crate::math::life::{Boundary, Config};
    fn moore() -> Cell2d {
