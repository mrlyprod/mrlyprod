...
/// assert!((mrlyrs::num::spin::ring(&solid, 4, 1.0) - 1.0).abs() < 1e-12);
/// assert!(mrlyrs::num::spin::ring(&solid, 4, 3.0).abs() < 1e-12);
/// ```
...
/// let rings = mrlyrs::num::spin::profile(&solid, 8, 4000);
/// let inner = mrlyrs::num::spin::mass_within(&rings, 8, 3.0);
