...
///
/// ```
/// let mut net = mrlyrs::num::graph::Network::new(2);
/// net.add_node(vec![0.0, 0.0]).unwrap();
/// net.add_node(vec![1.0, 0.0]).unwrap();
/// net.add_branch(0, 1, 1.0).unwrap();
/// assert_eq!(mrlyrs::num::graph::roles(&net), vec![mrlyrs::num::graph::Role::Tip; 2]);
/// ```
...
///
/// ```
/// let mut net = mrlyrs::num::graph::Network::new(1);
/// for i in 0..3 { net.add_node(vec![i as f64]).unwrap(); }
/// net.add_branch(0, 1, 1.0).unwrap();
/// assert_eq!(mrlyrs::num::graph::largest_component(&net).nodes.len(), 2);
/// ```
...
/// assert_eq!(mrlyrs::num::graph::census(&net).components, 2);
/// ```
...
mod tests {
    ...
    use crate::num::graph::extract::core_graph;
    #[test]
    fn carpet_census() {
