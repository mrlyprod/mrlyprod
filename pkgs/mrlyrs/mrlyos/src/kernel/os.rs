...
impl Os {
    ...
    pub fn uniforms(&self, app: &str) -> Option<Vec<f32>> {
        let i = self.find(app)?;
        self.apps[i].uniforms()
    }
    pub fn peek(&self, app: &str, shape: Option<&Json>) -> Option<View> {
        let i = self.find(app)?;
        let state = self.apps[i].state(&self.iden);
        let state = match shape {
            Some(s) => prune(&state, s),
            None => state,
        };
