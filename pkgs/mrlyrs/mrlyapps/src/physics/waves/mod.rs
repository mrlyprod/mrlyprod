use mrlycore::colors::{named, CYAN, PINK};
use mrlycore::rng::Rng;
use mrlycore::{json, Json};
use mrlymath::physics::waves::MILLI;
use mrlymath::physics::{Mask, Waves as Sim, WavesConfig};
...
impl Set {
    ...
    fn apply(&mut self, key: &str, value: &Json) -> Result<Json, &'static str> {
        match key {
            ...
            "speed" => int(&mut self.speed, value, (50, 450)),
            "damp" => int(&mut self.damp, value, (0, 20)),
            "freq" => int(&mut self.freq, value, (300, 4000)),
            "sigma" => int(&mut self.sigma, value, (1000, 6000)),
            "amp" => int(&mut self.amp, value, (400, 3000)),
            "reflect" => int(&mut self.reflect, value, (0, 1000)),
            _ => Err("no such key"),
        }
    }
...
impl App for Waves {
    ...
    fn load(&mut self, state: &Json) {
        ...
        if let Some(arr) = state["sources"].as_array() {
            for s in arr {
                if let (Some(x), Some(y)) = (s["x"].as_i64(), s["y"].as_i64()) {
                    self.sim.drop(x, y);
                }
            }
        }
