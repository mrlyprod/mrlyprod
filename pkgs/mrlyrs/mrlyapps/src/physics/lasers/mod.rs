use mrlycore::colors::{named, RED};
use mrlycore::rng::Rng;
use mrlycore::{json, Json};
use mrlymath::physics::lasers::{MILLI, WHEEL};
use mrlymath::physics::{Emitter, Lasers as Sim, LasersConfig, Mask};
...
const SPINS: [i64; 5] = [0, 4074, 12223, 32595, 81487];
...
impl Set {
    ...
    fn apply(&mut self, key: &str, value: &Json) -> Result<Json, &'static str> {
        match key {
            ...
            "spin" => {
                ...
                if !(0..SPINS.len() as i64).contains(&n) {
                    return Err("no such option");
                }
                self.spin = n;
                Ok(json!(n))
    ...
    fn config(&self) -> LasersConfig {
        ...
        LasersConfig {
            ...
            omega_idx: SPINS[self.spin.clamp(0, SPINS.len() as i64 - 1) as usize],
        }
    }
}
    ...
    fn act(&mut self, _iden: &Iden, call: &Call) -> Outcome {
        match call.verb.as_str() {
            "lasers.place" => {
                ...
                self.sim.spawn(x * MILLI, y * MILLI);
            ...
            let emitters: Option<Vec<Emitter>> = arr
                ...
                .map(|e| {
                    Some(Emitter {
                        x: e["x"].as_i64()?,
                        y: e["y"].as_i64()?,
                        dir: e["dir"].as_i64()?.rem_euclid(WHEEL),
                        omega_idx: e["omega_idx"].as_i64()?,
                        spread_idx: e["spread_idx"].as_i64()?,
...
mod tests {
    ...
    fn set_validates_mask_and_config_keys() {
        ...
        assert!(send(&mut l, "lasers.set", json!({ "key": "spin", "value": 3 })).ok);
        assert!(!send(&mut l, "lasers.set", json!({ "key": "spin", "value": 5 })).ok);
        assert!(!send(&mut l, "lasers.set", json!({ "key": "spin", "value": -1 })).ok);
        assert!(send(&mut l, "lasers.set", json!({ "key": "spin", "value": "2" })).ok);
        assert!(
            !send(
                &mut l,
                "lasers.set",
