...
use mrlymath::physics::billiards::{cell, solid_cell, CENTI, MILLI};
...
impl Set {
    ...
    fn apply(&mut self, key: &str, value: &Json) -> Result<Json, &'static str> {
        match key {
            ...
            "speed" => int(&mut self.speed, value, (50, 400)),
            "trail" => int(&mut self.trail, value, (2, 100)),
            "size" => int(&mut self.size, value, (100, 400)),
            _ => Err("no such key"),
        }
    }
...
impl Billiards {
    ...
    fn stamp(&mut self, x: i64, y: i64) {
        ...
        for dy in -ri..=ri {
            for dx in -ri..=ri {
                ...
                if (dx * dx + dy * dy) * CENTI * CENTI <= r * r {
                    let i = (py as usize) * (w as usize) + px as usize;
                    self.intensity[i] = FULL;
                }
            }
        }
    }
    fn render(&self) -> Frame {
        ...
        for y in 0..h {
            for x in 0..w {
                let i = y * w + x;
                if solid_cell(mask, x as i64, y as i64) {
                    colors[i] = WALL;
                    continue;
                }
                let t = self.intensity[i] as f64 / FULL as f64;
    ...
    fn act(&mut self, _iden: &Iden, call: &Call) -> Outcome {
        match call.verb.as_str() {
            "billiards.break" => {
                ...
                if solid_cell(self.sim.mask(), x, y) {
            ...
            "billiards.step" => {
                ...
                for _ in 0..n {
                    let keep = CENTI - self.set.trail;
                    for v in self.intensity.iter_mut() {
                        *v = (*v as i64 * keep / CENTI) as u8;
                    }
                    self.sim.step(MILLI);
    ...
    fn load(&mut self, state: &Json) {
        ...
        if let Some(arr) = state["trail"].as_array() {
            if arr.len() == self.intensity.len() {
                let values: Option<Vec<u8>> = arr
                    .iter()
                    .map(|v| v.as_i64().map(|n| n.clamp(0, FULL as i64) as u8))
                    .collect();
