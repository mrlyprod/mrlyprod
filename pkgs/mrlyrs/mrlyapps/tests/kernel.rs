mod tests {
    ...
    fn focused_state(os: &Os) -> Json {
        os.frame(None).view.unwrap().state
    }
    #[test]
    fn routes_to_first_app() {
        assert_eq!(boot().frame(None).route.unwrap().app, "notes");
    }
    #[test]
    fn act_flows_to_app() {
        let mut os = boot();
    ...
    fn frame_carries_the_open_routes_state() {
        ...
        let frame = os.frame(None).to_json();
    ...
    fn peek_reads_a_background_app() {
        ...
        let view = os.peek("notes", None).unwrap();
        ...
        assert!(os.peek("ghost", None).is_none());
    }
    #[test]
    fn verb_prefix_reaches_the_unfocused_app() {
        ...
        assert_eq!(
            os.peek("notes", None).unwrap().state["found"][0]["text"],
            "milk"
        );
    }
    #[test]
    ...
    fn nav_open_replaces_the_route() {
        ...
        assert_eq!(os.frame(None).route.unwrap().app, "home");
    ...
    fn nav_open_missing_fails() {
        ...
        assert_eq!(os.frame(None).route.unwrap().app, "notes");
    }
    #[test]
    fn envelope_serializes() {
        let j = boot().frame(None).to_json();
    ...
    fn kernel_remembers_now_and_stamps_the_unstamped() {
        ...
        assert_eq!(os.frame(None).last.unwrap().data["now"], json!(5000));
    }
    #[test]
    fn effects_move_to_the_envelope() {
        ...
        assert_eq!(os.frame(None).to_json()["effects"], Json::Null);
    }
    #[test]
    ...
    fn quiet_envelope_omits_notices() {
        assert_eq!(boot().frame(None).to_json()["notices"], Json::Null);
    }
    #[test]
    fn dismiss_clears_the_notice_log() {
        ...
        assert_eq!(os.frame(None).to_json()["notices"], Json::Null);
    }
    #[test]
    fn freeze_thaw_carries_notices() {
        ...
        let frame = b.frame(None).to_json();
        assert_eq!(frame["notices"][0]["title"], "beep");
        assert_eq!(frame["notices"][0]["at"], json!(5000));
        let mut c = boot();
        c.act(Call::new(
        ...
        assert_eq!(c.frame(None).to_json()["notices"], Json::Null);
    }
    #[test]
    fn beat_rides_the_focused_view() {
        let mut os = Os::new(Iden::new("aria"))
            ...
            .install(Box::new(Home));
        assert_eq!(
            os.frame(None).to_json()["view"]["beat"],
            json!({ "verb": "pulse.beep", "args": {} })
        );
        os.act(Call::new("nav.open", json!({ "app": "home" })));
        assert_eq!(os.frame(None).to_json()["view"]["beat"], Json::Null);
    }
    #[test]
    ...
    fn thaw_restores_a_fresh_boot() {
        ...
        let frame = b.frame(None);
        assert_eq!(frame.route.as_ref().unwrap().app, "home");
        assert!(b.act(Call::new("nav.open", json!({ "app": "notes" }))).ok);
        assert_eq!(b.frame(None).route.unwrap().app, "notes");
    ...
    fn thaw_restores_the_remembered_now() {
        ...
        assert_eq!(b.frame(None).last.unwrap().data["now"], json!(5000));
    }
    #[test]
    fn thaw_rejects_garbage() {
        ...
        assert!(out.ok);
        assert_eq!(os.frame(None).route, None);
        assert!(os.frame(None).view.is_none());
        assert!(os.act(Call::new("nav.open", json!({ "app": "notes" }))).ok);
        assert_eq!(os.frame(None).route.unwrap().app, "notes");
    }
    #[test]
