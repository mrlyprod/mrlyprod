...
import type { Pane, Side } from "./pane"
import { PANE_MAX, PANE_MIN, PANE_WIDE, clampPane, paintPane } from "./pane"
...
export function Grip({ side, pane }: { side: Side; pane: Pane }) {
  ...
  const down = (event: PointerEvent<HTMLDivElement>): void => {
    ...
    const move = (moved: globalThis.PointerEvent): void => {
      ...
      held.current = clampPane(start + step)
      paintPane(side, held.current)
    }
    const up = (): void => {
  ...
  const press = (event: KeyboardEvent<HTMLDivElement>): void => {
    const now = pane.width || PANE_WIDE
  ...
  return (
    <div
      ...
      aria-label={side === "left" ? "resize left pane" : "resize right pane"}
      aria-valuenow={pane.width || PANE_WIDE}
