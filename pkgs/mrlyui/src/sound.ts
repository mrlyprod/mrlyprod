...
export const freq = (midi: number): number => 440 * 2 ** ((midi - 69) / 12)

const shape = (wave: string): OscillatorType =>
  wave === "triangle" || wave === "square" || wave === "sawtooth" ? wave : "sine"

export function pref(key: "sound" | "haptics" | "note" | "wave" | "duration", value: unknown): void {
...
export function tap(): void {
  ...
  const midi =
    prefs.note === "random"
      ...
      : 48 + Math.max(0, NOTES.indexOf(prefs.note))
