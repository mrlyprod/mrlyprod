...
impl Tensor {
    ...
    ///
    /// ```
    /// let sponge = mrlycore::atoms::carpet_3d(3);
    /// assert_eq!(sponge.slice(2, 0).unwrap(), mrlycore::atoms::carpet_2d(3));
    /// ```
    pub fn slice(&self, axis: usize, index: usize) -> crate::Result<Tensor> {
    ...
    pub fn blur(&self, mask: &Tensor, wrap: bool) -> crate::Result<Tensor> {
        use crate::errors::value_error;
        if mask.shape.len() != self.shape.len() {
            return value_error("mask must have the same number of dimensions.");
        }
    ...
    pub fn perforate(&self, mask: &Tensor, value: u8) -> crate::Result<Tensor> {
        use crate::errors::value_error;
        if mask.shape.len() != self.shape.len() {
            return value_error("mask must have the same number of dimensions.");
        }
...
mod tests {
    ...
    fn slice_commutes_with_the_fractal() {
        for (n, level) in [(3, 2), (5, 2), (3, 3)] {
            let seed = crate::atoms::xtree_3d(n);
            for axis in 0..3 {
