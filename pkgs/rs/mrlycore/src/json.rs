...
/// A JSON value restricted to null, booleans, integers, strings, arrays and objects.
...
impl Json {
    ...
    /// Returns the integer unsigned, or None when negative or not an integer.
    ...
    /// Returns the array, or None for any other kind.
    ...
    /// Returns the array mutably, or None for any other kind.
    ...
    /// Returns the object, or None for any other kind.
    ...
    /// Returns the object mutably, or None for any other kind.
    ...
    /// Returns the value at a key or position, or None when it does not resolve.
    pub fn get<I: JsonIndex>(&self, index: I) -> Option<&Json> {
        index.index_into(self)
    }

    /// Returns the value at a key or position mutably, or None when it does not resolve.
...
impl Map {
    ...
    /// Sets a key to a value, returning the value it replaced.
    ...
    /// Returns the value at a key, or None when absent.
    pub fn get(&self, key: &str) -> Option<&Json> {
        self.entries.iter().find(|(k, _)| k == key).map(|(_, v)| v)
    }

    /// Returns the value at a key mutably, or None when absent.
    ...
    /// Opens a key's slot for insert-if-absent access.
    ...
    /// Iterates the entries mutably in insertion order.
    ...
    /// Iterates the values mutably in insertion order.
    pub fn values_mut(&mut self) -> impl Iterator<Item = &mut Json> {
        self.entries.iter_mut().map(|(_, v)| v)
    }
}

/// One key's pending slot in a map.
pub struct Entry<'a> {
    map: &'a mut Map,
    key: String,
}
...
impl<'a> Entry<'a> {
    /// Returns the slot's value, inserting the default when the key is absent.
    pub fn or_insert(self, default: Json) -> &'a mut Json {
        self.or_insert_with(|| default)
    }

    /// Returns the slot's value, inserting the closure's result when the key is absent.
...
/// The index into a value, by object key or array position.
pub trait JsonIndex {
    /// Returns the indexed value, or None when it does not resolve.
    fn index_into<'a>(&self, value: &'a Json) -> Option<&'a Json>;
    /// Returns the indexed value mutably, or None when it does not resolve.
    fn index_into_mut<'a>(&self, value: &'a mut Json) -> Option<&'a mut Json>;
    /// Returns the indexed value mutably, creating the slot or panicking where it cannot.
...
/// Parses JSON text into a value, or an error naming the byte where it broke.
///
/// ```
...
/// Folds JSON-shaped tokens into a Json value on behalf of the json macro.
#[macro_export]
#[doc(hidden)]
