...
/// The fragment shader the set is drawn with, banding escape counts between the two colors.
pub const FRAGMENT: &str = include_str!("mandelbrot.wgsl");
...
/// One seeded journey through the set: a dive in progress, its dials and its palette phase.
...
impl Mandelbrot {
    /// Frames the whole set on seed zero, aimed at its first target.
