...
/// The three-pen tile skin the head, the body and the food are drawn with.
pub mod skin;
...
/// A round of snake on a square grid, laid out and replayed from a seed.
pub struct Snake {
    set: Set,
    rng: Rng,
    seed: u64,
...
impl Snake {
    /// Deals a fresh round on seed zero: two segments centered, one apple down.
    pub fn new() -> Snake {
        let mut snake = Snake {
            set: Set::new(),
            rng: Rng::new(0),
