...
/// The picture skins: generated sprites in rolled colors or plain ink.
pub mod skin;
...
/// One seeded run of the puzzle: the grid, its hidden target and the score.
pub struct Captcha {
    set: Set,
    rng: Rng,
    seed: u64,
...
impl Captcha {
    /// Deals a fresh puzzle from seed 0.
    pub fn new() -> Captcha {
    ...
    fn grade(&mut self, idx: usize) -> Outcome {
        ...
        if idx == self.target {
            self.score += 1;
            self.roll();
            Outcome::ok(json!({ "correct": true })).emit(Effect::new("sound", audio::cue("good")))
        } else {
            self.over = true;
            Outcome::ok(json!({ "correct": false })).emit(Effect::new("sound", audio::cue("bad")))
        }
    }
}
