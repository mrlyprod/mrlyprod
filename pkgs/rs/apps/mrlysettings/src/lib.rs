...
/// The two shapes the home screen can take.
pub const MODES: [&str; 2] = ["grid", "list"];

/// The five type faces the system can be set in.
pub const FONTS: [&str; 5] = ["mono", "sans", "serif", "display", "mrly"];

/// The two sets an emoji glyph can be drawn from.
pub const EMOJIS: [&str; 2] = ["system", "noto"];

/// The two finishes a surface can be given.
pub const MATERIALS: [&str; 2] = ["solid", "glass"];

/// The two kinds of backdrop, a flat color or a generated pattern.
pub const WALLPAPERS: [&str; 2] = ["color", "pattern"];

/// The one control panel, shared with every app at once.
pub struct Settings {
...
impl Settings {
    ...
    /// Builds the panel with every key at its default.
    pub fn new() -> Settings {
        Settings {
...
impl App for Settings {
    ...
    fn actions(&self, _iden: &Iden) -> Vec<Verb> {
        vec![Verb::new(
            ...
            json!({
                "key": {
                    "launchpad": MODES.join(" | "),
                    "darkmode": "bool",
                    "color": COLORS.join(" | "),
                    "fill": format!("random | {}", COLORS.join(" | ")),
                    "font": FONTS.join(" | "),
                    "emoji": EMOJIS.join(" | "),
                    "scale": "int 3..6",
                    "radius": "int 0..4",
                    ...
                    "sound": "bool",
                    "haptics": "bool",
                    "note": format!("random | {}", mrlycore::audio::NOTES.join(" | ")),
                    "wave": mrlycore::audio::WAVES.join(" | "),
                    "duration": "int 50..1000",
                    "background": COLORS.join(" | "),
                    "width": "int 500..1500",
                    "material": MATERIALS.join(" | "),
                    "wallpaper": WALLPAPERS.join(" | "),
                    "seed": "int 0..999",
                    "detail": "int 32..160",
                },
                "value": "of key",
