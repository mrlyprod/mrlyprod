...
/// A time of day split into hours, minutes and seconds.
pub struct Time {
...
impl Time {
    /// Builds a time, wrapping each part back into its own range.
    ...
    /// Splits a second count into a time of day, wrapping at a full day.
    ...
    /// Renders the time zero-padded, 09:05:03 style.
    pub fn text(&self) -> String {
        format!("{:02}:{:02}:{:02}", self.hour, self.minute, self.second)
    }
}

pub use mrlycore::time::civil;

/// The clock app: the last moment the heartbeat handed over, in milliseconds.
pub struct Clock {
    now: i64,
}
...
impl Clock {
    /// Builds a clock that has never been set.
    pub fn new() -> Clock {
        Clock { now: 0 }
    }
}
