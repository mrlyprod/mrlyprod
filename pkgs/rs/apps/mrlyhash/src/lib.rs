...
/// The two-pen tile skin the fingerprint is drawn with.
pub mod skin;
...
/// The digest bench: one text held under one automaton rule.
pub struct Hash {
    text: String,
    rule: Rule,
    dark: bool,
...
impl Hash {
    /// Builds the bench on the text mrly under the life rule.
    pub fn new() -> Hash {
        Hash {
            text: "mrly".to_string(),
            rule: Rule::Life,
    ...
    fn actions(&self, _iden: &Iden) -> Vec<Verb> {
        vec![
            ...
            Verb::new(
                "hash.set",
                json!({
                    "key": { "rule": "life | maze | replicator | anneal" },
                    "value": "of key",
                }),
            ),
