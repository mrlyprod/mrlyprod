...
/// The countdown and stopwatch sharing one face: a deadline, a banked stopwatch, its laps and whether it has rung.
pub struct Timer {
    stopwatch: bool,
    deadline: Option<i64>,
    paused: Option<i64>,
...
impl Timer {
    /// Builds the timer idle in countdown mode.
    pub fn new() -> Timer {
        Timer {
            stopwatch: false,
            deadline: None,
...
impl App for Timer {
    ...
    fn actions(&self, _iden: &Iden) -> Vec<Verb> {
        ...
        out.push(Verb::new(
            "timer.set",
            json!({
                "key": { "duration": { "h": "int 0..24", "m": "int 0..1440" } },
                "value": "of key",
            }),
        ));
        if self.stopwatch {
