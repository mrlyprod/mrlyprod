#![doc = include_str!("../README.md")]
#![deny(missing_docs)]

/// The sixteen-pen tile skin the canvas is drawn with.
pub mod skin;

use mrlycore::colors::hex;
use mrlycore::colors::{BLACK, ROLLABLE};
use mrlycore::rng::Rng;
use mrlycore::{json, Json};
use mrlyos::kernel::{App, Call, Iden, Manifest, Outcome, Verb};

struct Set {
    width: i64,
    height: i64,
    tile: i64,
}

impl Set {
    fn new() -> Set {
        Set {
            width: 24,
            height: 24,
            tile: 3,
        }
    }
    fn apply(&mut self, key: &str, value: &Json) -> Result<Json, &'static str> {
        match key {
            "width" | "height" | "tile" => {
                let n = value.as_i64().ok_or("value must be an integer")?;
                let (min, max) = match key {
                    "width" => (4, 64),
                    "height" => (4, 64),
                    _ => (1, 8),
                };
                if !(min..=max).contains(&n) {
                    return Err("out of range");
                }
                match key {
                    "width" => self.width = n,
                    "height" => self.height = n,
                    _ => self.tile = n,
                }
                Ok(json!(n))
            }
            _ => Err("no such key"),
        }
    }
    fn to_json(&self) -> Json {
        json!({
            "width": self.width,
            "height": self.height,
            "tile": self.tile,
        })
    }
    fn from_json(value: &Json) -> Set {
        let mut set = Set::new();
        if let Some(obj) = value.as_object() {
            for (key, val) in obj {
                let _ = set.apply(key, val);
            }
        }
        set
    }
}

/// The paint app: a grid of on-and-off cells drawn in one ink rolled from the seed.
pub struct Paint {
    set: Set,
    rng: Rng,
    seed: u64,
    steps: u64,
    canvas: Vec<bool>,
    ink_color: [u8; 4],
}

impl Default for Paint {
    fn default() -> Paint {
        Paint::new()
    }
}

impl Paint {
    /// Opens a blank 24 by 24 canvas on seed zero with its ink already rolled.
    pub fn new() -> Paint {
        let mut paint = Paint {
            set: Set::new(),
            rng: Rng::new(0),
            seed: 0,
            steps: 0,
            canvas: Vec::new(),
            ink_color: [255, 255, 255, 255],
        };
        paint.reset(0);
        paint
    }
    fn width(&self) -> usize {
        self.set.width as usize
    }
    fn height(&self) -> usize {
        self.set.height as usize
    }
    fn size(&self) -> usize {
        self.width() * self.height()
    }
    fn painted(&self) -> u64 {
        self.canvas.iter().filter(|&&p| p).count() as u64
    }
    fn palette(&mut self) -> [u8; 4] {
        let c = ROLLABLE[self.rng.below(ROLLABLE.len())];
        [c.r, c.g, c.b, 255]
    }
    fn parse_points(&self, value: &Json) -> Result<Vec<(usize, usize)>, &'static str> {
        let arr = value
            .as_array()
            .ok_or("points must be an array of [x, y] pairs")?;
        let (w, h) = (self.width() as i64, self.height() as i64);
        let mut out = Vec::with_capacity(arr.len());
        for p in arr {
            let pair = p
                .as_array()
                .ok_or("points must be an array of [x, y] pairs")?;
            if pair.len() != 2 {
                return Err("points must be an array of [x, y] pairs");
            }
            let x = pair[0]
                .as_i64()
                .ok_or("points must be an array of [x, y] pairs")?;
            let y = pair[1]
                .as_i64()
                .ok_or("points must be an array of [x, y] pairs")?;
            if !(0..w).contains(&x) || !(0..h).contains(&y) {
                return Err("point out of bounds");
            }
            out.push((x as usize, y as usize));
        }
        Ok(out)
    }
    fn canvas_facts(&self) -> Vec<Vec<bool>> {
        let w = self.width();
        (0..self.height())
            .map(|r| (0..w).map(|c| self.canvas[r * w + c]).collect())
            .collect()
    }
    fn ids(&self) -> Vec<Vec<u8>> {
        let w = self.width();
        (0..self.height())
            .map(|r| (0..w).map(|c| u8::from(self.canvas[r * w + c])).collect())
            .collect()
    }
    fn pens(&self) -> Vec<String> {
        let mut out = vec![hex([0, 0, 0, 0]), hex(self.ink_color)];
        for c in ROLLABLE {
            out.push(hex([c.r, c.g, c.b, 255]));
        }
        out.push(hex([BLACK.r, BLACK.g, BLACK.b, 255]));
        out
    }
    fn cells_fact(&self) -> Json {
        json!({
            "ids": self.ids(),
            "skin": "tiles",
            "pens": self.pens(),
        })
    }
    fn reset(&mut self, seed: u64) {
        self.rng = Rng::new(seed);
        self.seed = seed;
        self.steps = 0;
        self.canvas = vec![false; self.size()];
        self.ink_color = self.palette();
    }
    fn clear(&mut self) {
        self.canvas = vec![false; self.size()];
    }
}

impl App for Paint {
    fn route(&self) -> &str {
        "paint"
    }
    fn manifest(&self) -> Manifest {
        Manifest::new("paint").emoji("🎨").category("design")
    }
    fn state(&self, _iden: &Iden, _shape: Option<&Json>) -> Json {
        json!({
            "score": 0,
            "steps": self.steps,
            "over": false,
            "seed": self.seed,
            "settings": self.set.to_json(),
            "canvas": self.canvas_facts(),
            "painted": self.painted(),
            "cells": self.cells_fact(),
        })
    }
    fn actions(&self, _iden: &Iden) -> Vec<Verb> {
        vec![
            Verb::new(
                "paint.stroke",
                json!({
                    "points": format!("points 0..{} 0..{}", self.width() - 1, self.height() - 1),
                }),
            ),
            Verb::new("paint.clear", json!({})),
            Verb::new("paint.reset", json!({ "seed": "int" })),
            Verb::new(
                "paint.set",
                json!({
                    "key": {
                        "width": "int 4..64",
                        "height": "int 4..64",
                        "tile": "int 1..8",
                    },
                    "value": "of key",
                }),
            ),
        ]
    }
    fn call(&mut self, _iden: &Iden, call: &Call) -> Outcome {
        match call.verb.as_str() {
            "paint.stroke" => match self.parse_points(call.arg("points")) {
                Ok(points) => {
                    let w = self.width();
                    for (x, y) in &points {
                        self.canvas[y * w + x] = true;
                    }
                    self.steps += 1;
                    Outcome::ok(json!({ "points": points.len() }))
                }
                Err(note) => Outcome::fail(note),
            },
            "paint.clear" => {
                self.clear();
                self.steps += 1;
                Outcome::ok(json!({ "cleared": true }))
            }
            "paint.reset" => {
                let seed = call
                    .arg("seed")
                    .as_u64()
                    .unwrap_or(call.now.unwrap_or(0).max(0) as u64);
                self.reset(seed);
                Outcome::ok(json!({ "seed": seed }))
            }
            "paint.set" => {
                let key = call.arg("key").as_str().unwrap_or("").to_string();
                match self.set.apply(&key, call.arg("value")) {
                    Ok(value) => {
                        let seed = self.seed;
                        self.reset(seed);
                        Outcome::ok(json!({ "key": key, "value": value }))
                    }
                    Err(note) => Outcome::fail(note),
                }
            }
            _ => Outcome::fail("unknown verb"),
        }
    }
    fn save(&self) -> Json {
        json!({
            "settings": self.set.to_json(),
            "seed": self.seed,
            "pos": self.rng.pos() as u64,
            "steps": self.steps,
            "canvas": self.canvas.clone(),
        })
    }
    fn load(&mut self, state: &Json) {
        self.set = Set::from_json(&state["settings"]);
        self.reset(state["seed"].as_u64().unwrap_or(0));
        if let Some(arr) = state["canvas"].as_array() {
            let canvas: Option<Vec<bool>> = arr.iter().map(|v| v.as_bool()).collect();
            if let Some(canvas) = canvas {
                if canvas.len() == self.size() {
                    self.canvas = canvas;
                    self.steps = state["steps"].as_u64().unwrap_or(0);
                }
            }
        }
        if let Some(pos) = state["pos"].as_u64() {
            self.rng.seek(pos as u128);
        }
    }
}

#[cfg(test)]
mod app_tests {
    use super::*;
    use mrlyos::kernel::testkit::{iden, seeded, send};

    fn paint(seed: u64) -> Paint {
        seeded(Paint::new(), "paint.reset", seed)
    }

    #[test]
    fn seed_reproduces() {
        let mut a = paint(3);
        let mut b = paint(3);
        for p in [&mut a, &mut b] {
            send(p, "paint.stroke", json!({ "points": [[1, 1], [2, 2]] }));
        }
        assert_eq!(a.state(&iden(), None), b.state(&iden(), None));
        assert_eq!(a.save(), b.save());
    }
    #[test]
    fn stroke_draws_and_clear_wipes() {
        let mut p = paint(1);
        let out = send(
            &mut p,
            "paint.stroke",
            json!({ "points": [[0, 0], [1, 0], [2, 0]] }),
        );
        assert!(out.ok);
        assert_eq!(out.data["points"], json!(3));
        let state = p.state(&iden(), None);
        assert_eq!(state["cells"]["ids"][0][0], json!(1));
        assert_eq!(state["cells"]["ids"][0][3], json!(0));
        assert_eq!(state["painted"], json!(3));
        let out = send(&mut p, "paint.clear", json!({}));
        assert!(out.ok);
        let state = p.state(&iden(), None);
        assert_eq!(state["painted"], json!(0));
        assert_eq!(state["cells"]["ids"][0][0], json!(0));
    }
    #[test]
    fn malformed_points_fail_honestly() {
        let mut p = paint(1);
        assert!(!send(&mut p, "paint.stroke", json!({ "points": "nope" })).ok);
        assert!(!send(&mut p, "paint.stroke", json!({ "points": [[1]] })).ok);
        assert!(!send(&mut p, "paint.stroke", json!({ "points": [[1, "x"]] })).ok);
        assert!(!send(&mut p, "paint.stroke", json!({ "points": [[999, 0]] })).ok);
    }
    #[test]
    fn reset_seed_defaults_to_now() {
        let mut p = Paint::new();
        let out = p.call(&iden(), &Call::new("paint.reset", json!({})).at(5000));
        assert!(out.ok);
        assert_eq!(out.data["seed"], json!(5000));
        assert_eq!(p.state(&iden(), None)["seed"], json!(5000));
    }
    #[test]
    fn set_validates_and_resets_the_round() {
        let mut p = paint(4);
        send(&mut p, "paint.stroke", json!({ "points": [[0, 0]] }));
        let out = send(&mut p, "paint.set", json!({ "key": "width", "value": 8 }));
        assert!(out.ok);
        let state = p.state(&iden(), None);
        assert_eq!(state["settings"]["width"], json!(8));
        assert_eq!(state["steps"], json!(0));
        assert_eq!(state["painted"], json!(0));
        assert!(!send(&mut p, "paint.set", json!({ "key": "width", "value": 999 })).ok);
        assert!(!send(&mut p, "paint.set", json!({ "key": "volume", "value": 1 })).ok);
    }
    #[test]
    fn save_load_roundtrips_and_continues() {
        let mut a = paint(11);
        send(
            &mut a,
            "paint.stroke",
            json!({ "points": [[1, 1], [2, 2]] }),
        );
        let mut b = Paint::new();
        b.load(&a.save());
        assert_eq!(b.state(&iden(), None), a.state(&iden(), None));
        assert_eq!(b.save(), a.save());
        for p in [&mut a, &mut b] {
            send(p, "paint.stroke", json!({ "points": [[3, 3]] }));
        }
        assert_eq!(b.state(&iden(), None), a.state(&iden(), None));
    }
    #[test]
    fn load_survives_garbage() {
        let mut p = Paint::new();
        p.load(&json!({ "seed": "soup", "canvas": [1, 2, 3], "settings": 7 }));
        assert_eq!(p.state(&iden(), None)["steps"], json!(0));
        assert_eq!(p.state(&iden(), None)["seed"], json!(0));
    }
    #[test]
    fn actions_offer_the_natural_verbs() {
        let p = paint(3);
        let names: Vec<String> = p.actions(&iden()).iter().map(|v| v.name.clone()).collect();
        assert_eq!(
            names,
            vec!["paint.stroke", "paint.clear", "paint.reset", "paint.set"]
        );
    }
    #[test]
    fn state_carries_the_cells_fact() {
        let p = paint(5);
        let state = p.state(&iden(), None);
        let cells = &state["cells"];
        assert_eq!(cells["skin"], json!("tiles"));
        assert_eq!(cells["ids"].as_array().unwrap().len(), 24);
        assert_eq!(cells["ids"][0].as_array().unwrap().len(), 24);
        let pens = cells["pens"].as_array().unwrap();
        assert_eq!(pens.len(), 16);
        assert!(pens.iter().all(|p| p.as_str().unwrap().starts_with('#')));
        assert_eq!(pens[0], json!("#00000000"));
        assert!(state.get("frame").is_none());
        assert!(state.get("sprites").is_none());
        assert_eq!(state["score"], json!(0));
        assert_eq!(state["over"], json!(false));
    }
}
