...
/// The English month names, January through December.
...
/// The weekday initials, Monday first.
pub const DAYS: [&str; 7] = ["M", "T", "W", "T", "F", "S", "S"];

/// Reports whether a year gets a February 29.
pub fn leap(year: i32) -> bool {
    (year % 4 == 0 && year % 100 != 0) || year % 400 == 0
}

/// Counts the days in a month, 0 for a month that does not exist.
...
/// Gives the day of the week for a date, 0 for Monday through 6 for Sunday.
...
pub struct Cell {
    /// The day number written in the square.
    pub day: u32,
    /// Whether the day is borrowed from a neighbouring month.
    pub faded: bool,
}

/// The month view: the page on show, the last known time, and the picked day.
pub struct Calendar {
    year: i32,
    month: u32,
    now: i64,
...
impl Calendar {
    /// Opens the calendar on January 1970.
    pub fn new() -> Calendar {
        Calendar {
            year: 1970,
            month: 1,
    ...
    /// Writes the page heading, month name then year.
    ...
    /// Lays the month out as whole weeks, borrowed days marked faded.
