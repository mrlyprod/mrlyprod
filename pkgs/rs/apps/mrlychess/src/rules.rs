...
/// How a team stands when the move is theirs.
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub enum Status {
    /// The team has a legal move and its king is not attacked.
    Normal,
    /// The team's king is attacked and a legal move remains.
    Check,
    /// The team's king is attacked and no legal move remains.
    Checkmate,
    /// The team has no legal move and its king is not attacked.
    Stalemate,
}
...
impl Chess {
    ...
    /// Finds where a team's king stands, if that team has one at all.
    ...
    /// Reports whether a team's king is unattacked; a missing king is never safe.
    ...
    /// Lists every square a piece may legally reach, castling included and pins ruled out.
    ...
    /// Plays a move, carrying out en passant, castling and promotion, and arms the next en passant square.
    ...
    /// Reports whether a move would land a pawn on a far rank.
    pub fn promoting(&self, from: usize, to: usize) -> bool {
        let (_, ty) = self.coords(to);
        self.board[from].kind == 1 && (ty == 0 || ty == self.h - 1)
    }
    /// Judges a team's position as normal, check, checkmate or stalemate.
