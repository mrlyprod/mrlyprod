...
/// The two looks a piece can take.
pub const VARIANTS: [&str; 2] = ["digits", "emojis"];

/// The roles one square parity claims: the bare square plus six pieces a side.
...
/// Builds the skin for one variant, covering both square parities and both teams.
pub fn skin(variant: &str) -> Skin {
    let mut visuals = Vec::new();
    for parity in 0..2usize {
        visuals.push(Visual::pen(2 + parity));
        for team in 0..2usize {
            for (glyphs, emojis) in GLYPHS.iter().zip(EMOJIS) {
                let square = Visual::pen(2 + parity);
                visuals.push(match variant {
                    "emojis" => square.glyph(emojis[team]).tinted(Tint::Pen(team)),
                    _ => square.sprite(rows(glyphs)).tinted(Tint::Pen(team)),
                });
            }
        }
    }
...
/// Lists every variant beside its skin.
