...
fn cells_raster_paints_the_board() {
    let g = game();
    let cells = g.state(&iden(), None)["cells"].clone();
    let frame = mrlyskin::raster(crate::skin::corpus(), &cells, 5, false).expect("a frame");
...
fn moves_emit_cues() {
    ...
    let mut m = custom("7k/8/8/6Q1/8/8/8/K7");
    let out = mv(&mut m, "g5", "g6");
    assert_eq!(out.effects[0].data, audio::cue("lose"));
}
#[test]
...
fn surface_and_skin_validate() {
    ...
    let cells = g.state(&iden(), None)["cells"].clone();
    let shot = mrlyskin::raster(crate::skin::corpus(), &cells, 5, false).expect("a raster");
