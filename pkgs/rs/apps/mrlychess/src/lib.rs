...
mod rules;
mod setup;
/// The piece skins the squares are drawn with, as sprites or as chess glyphs.
pub mod skin;
...
/// The chess app: one board, both sides played by hand, every rule enforced.
pub struct Chess {
    set: Set,
    rng: Rng,
    seed: u64,
...
impl Chess {
    /// Deals the default layout on seed zero, with white to move.
    pub fn new() -> Chess {
        let set = Set::new();
    ...
    /// Returns the square index at a file and rank counted from the top left.
    pub fn cell(&self, x: usize, y: usize) -> usize {
        y * self.w + x
    }
    /// Splits a square index back into its file and rank.
    pub fn coords(&self, c: usize) -> (usize, usize) {
        (c % self.w, c / self.w)
    }
    /// Reports whether a signed file and rank still land on the board.
    pub fn bound(&self, x: i32, y: i32) -> bool {
        x >= 0 && x < self.w as i32 && y >= 0 && y < self.h as i32
    }
    /// Returns how many squares the dealt board holds.
    pub fn n(&self) -> usize {
        self.w * self.h
    }
    /// Deals the layout afresh on a new seed, clearing the game and rolling new colors.
    ...
    fn perform(&mut self, from: usize, to: usize, promote: u8) -> Outcome {
        ...
        .emit(Effect::new("sound", audio::cue(name)))
    }
