...
/// The tile skin the maze is drawn with.
pub mod skin;
...
impl Set {
    ...
    fn apply(&mut self, key: &str, value: &Json) -> Result<Json, &'static str> {
        match key {
            ...
            "reward_food" => int(&mut self.reward_food, value, (0, 1000)),
...
/// The escape app: a seeded maze run with ghosts hunting the shortest path.
pub struct Escape {
    set: Set,
    rng: Rng,
    seed: u64,
...
impl Escape {
    /// Builds a run on seed zero, standing at the center of the maze that seed picks.
    pub fn new() -> Escape {
...
impl App for Escape {
    ...
    fn manifest(&self) -> Manifest {
        Manifest::new("escape")
            .emoji("👻")
            .category("games")
            .key("up", Call::new("escape.turn", json!({ "dir": "up" })))
            .key("down", Call::new("escape.turn", json!({ "dir": "down" })))
    ...
    fn actions(&self, _iden: &Iden) -> Vec<Verb> {
        ...
        out.push(Verb::new(
            ...
            json!({
                "key": {
                    "map": MAP_CHOICES.join(" | "),
                    "ghost_ratio": "int 1..4",
                    "speed": "int 1..4",
                    "tile": "int 1..8",
                    "reward_food": "int 0..1000",
                    "reward_win": "int 0..10000",
                    "reward_lose": "int -10000..0",
                    "reward_step": "int -1000..1000",
                    "design": DESIGNS.join(" | "),
                },
                "value": "of key",
            }),
    ...
    fn call(&mut self, _iden: &Iden, call: &Call) -> Outcome {
        match call.verb.as_str() {
            ...
            "escape.step" => {
                ...
                if ate {
                    out = out.emit(Effect::new("sound", audio::cue("blip")));
                }
                if leveled {
                    out = out.emit(Effect::new("sound", audio::cue("good")));
                }
                if self.over {
...
mod tests {
    ...
    fn caught_ends_the_run() {
        ...
        assert!(out
            .effects
            .contains(&Effect::new("sound", audio::cue("lose"))));
