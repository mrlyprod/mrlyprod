...
/// The die-face skins: tiles, emojis and digits.
pub mod skin;
...
/// The side counts of the seven dice in the cup.
pub const SIDES: [u32; 7] = [2, 4, 6, 8, 10, 12, 20];

const SURFACES: [&str; 2] = ["grid", "canvas"];
const HISTORY: usize = 8;

/// The dice app: a seeded cup rolling one die at a time, with a short history.
pub struct Dice {
    rng: Rng,
    seed: u64,
    steps: u64,
...
impl Dice {
    /// Builds a fresh cup: a d6 on seed zero with no rolls yet.
    pub fn new() -> Dice {
        Dice {
            rng: Rng::new(0),
            seed: 0,
    ...
    fn actions(&self, _iden: &Iden) -> Vec<Verb> {
        vec![
            ...
            Verb::new(
                ...
                json!({
                    "key": {
                        "sides": SIDES.map(|n| n.to_string()).join(" | "),
...
mod tests {
    ...
    fn roll_blips() {
        ...
        assert_eq!(out.effects[0].data, audio::cue("blip"));
    }
    #[test]
    ...
    fn look_keys_take_emojis_on_either_surface() {
        ...
        let frame =
            mrlyskin::raster(crate::skin::corpus(), &state["cells"], 12, false).expect("a frame");
        assert_eq!(frame.width(), mrlyskin::SYMBOL);
