...
/// The card-face skins: tiles, emojis and digits.
pub mod skin;

use crate::skin::FACES;
use mrlycore::audio;
...
impl Set {
    ...
    fn apply(&mut self, key: &str, value: &Json) -> Result<Json, &'static str> {
        match key {
            ...
            "reward_match" => int(&mut self.reward_match, value, (0, 10000)),
            "reward_miss" => int(&mut self.reward_miss, value, (-1000, 0)),
            "sudden" => {
...
/// One seeded run of the game: the round on the table, what has been turned over and the score so far.
pub struct Memory {
    set: Set,
    rng: Rng,
    seed: u64,
...
impl Memory {
    /// Deals four pairs and a decoy on seed zero, face up for the look.
    pub fn new() -> Memory {
        ...
        out.push(Verb::new(
            ...
            json!({
                "key": {
                    ...
                    "cols": "int 2..8",
                    "tile": "int 1..8",
                    "reward_match": "int 0..10000",
                    "reward_miss": "int -1000..0",
                    "sudden": "bool",
                    "design": DESIGNS.join(" | "),
                ...
                match self.first {
                    None => {
                        ...
                        Outcome::ok(json!({ "card": card, "matched": false }))
                            .emit(Effect::new("sound", audio::cue("blip")))
                    }
                    Some(first) => {
                        self.first = None;
                        if self.faces[first] == self.faces[card] {
                            ...
                            if self.won() {
                                ...
                                .emit(Effect::new("sound", audio::cue("win")));
                            }
                            Outcome::ok(json!({ "card": card, "matched": true }))
                                .emit(Effect::new("sound", audio::cue("good")))
                        } else if self.set.sudden {
                            self.over = true;
                            self.peek = vec![first, card];
                            Outcome::ok(json!({ "card": card, "matched": false, "over": true }))
                                .emit(Effect::new("sound", audio::cue("lose")))
                        } else {
                            self.peek = vec![first, card];
                            Outcome::ok(json!({ "card": card, "matched": false }))
                                .emit(Effect::new("sound", audio::cue("bad")))
                        }
                    }
                }
            }
...
mod tests {
    ...
    fn sudden_death_ends_on_a_mismatch() {
        ...
        assert!(out.ok);
        assert!(m.over);
        assert_eq!(out.effects[0].data, mrlycore::audio::cue("lose"));
        assert!(!send(&mut m, "memory.flip", json!({ "card": 0 })).ok);
    }
    #[test]
    ...
    fn emojis_ride_either_surface() {
        ...
        let frame =
            mrlyskin::raster(crate::skin::corpus(), &state["cells"], 4, false).expect("a frame");
        assert_eq!(frame.width(), m.cols() * mrlyskin::SYMBOL);
        assert!(
            !send(
                &mut m,
                "memory.set",
