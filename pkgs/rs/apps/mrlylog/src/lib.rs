...
/// The session transcript: a mirror of the world's action ring, read back newest first and saving nothing of its own.
pub struct Log {
    ring: Vec<Json>,
}
...
impl Log {
    /// Builds an empty transcript, holding nothing until a session is worn.
    pub fn new() -> Log {
        Log { ring: Vec::new() }
    }
}
    ...
    fn call(&mut self, _iden: &Iden, call: &Call) -> Outcome {
        match call.verb.as_str() {
            "log.export" => {
                ...
                let data = mrlycore::base64(md.as_bytes());
...
mod tests {
    ...
    fn export_emits_a_markdown_file() {
        ...
        assert_eq!(
            effect.data["data"],
            json!(mrlycore::base64(expected.as_bytes()))
        );
    }
}
