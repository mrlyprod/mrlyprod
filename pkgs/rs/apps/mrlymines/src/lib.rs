...
/// The field skins: tiles, emojis and digits.
pub mod skin;

use crate::skin::VARIANTS;
use mrlycore::audio;
...
/// The mines app: a seeded field whose mines are only laid once the first cell is dug.
pub struct Mines {
    set: Set,
    rng: Rng,
    seed: u64,
...
impl Mines {
    /// Builds a fresh nine by nine field of ten mines on seed zero.
    pub fn new() -> Mines {
        let mut mines = Mines {
            set: Set::new(),
            rng: Rng::new(0),
...
mod tests {
    ...
    fn emojis_ride_either_surface() {
        ...
        let frame =
            mrlyskin::raster(crate::skin::corpus(), &state["cells"], 3, false).expect("a frame");
        assert_eq!(frame.width(), 9 * mrlyskin::SYMBOL);
        assert!(
            !send(
                &mut m,
                "mines.set",
    ...
    fn moves_ring_the_right_cues() {
        ...
        assert_eq!(out.effects[0].data, mrlycore::audio::cue("blip"));
