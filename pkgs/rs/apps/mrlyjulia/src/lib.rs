...
/// The shader source that draws the set on a GPU.
pub const FRAGMENT: &str = include_str!("julia.wgsl");

/// How many uniform floats the shader is fed each frame.
...
/// One seeded dive: the fall toward a spot near the set's edge, its dials and its palette phase.
pub struct Julia {
    set: Set,
    rng: Rng,
    seed: u64,
...
impl Julia {
    /// Begins a dive on seed zero, at the first preset constant.
    pub fn new() -> Julia {
        let mut julia = Julia {
            set: Set::new(),
            rng: Rng::new(0),
...
impl App for Julia {
    ...
    fn actions(&self, _iden: &Iden) -> Vec<Verb> {
        let swatch = mrlycore::colors::PALETTE
            .iter()
            .map(Color::to_hex)
            .collect::<Vec<String>>()
            .join(" | ");
            ...
            Verb::new(
                ...
                json!({
                    "key": {
                        ...
                        "depth": "int 16..600",
                        "preset": PRESETS.join(" | "),
                        "cre": format!("int {}..{}", -2 * FEMTO, 2 * FEMTO),
                        "cim": format!("int {}..{}", -2 * FEMTO, 2 * FEMTO),
                        "zoom": "int 1000..1050",
                        "cycle": "int 30..3000",
                        "start": "int 250..4000",
                        "band": "int 2000..64000",
