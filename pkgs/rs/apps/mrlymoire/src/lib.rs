...
/// The fragment shader that sums the two lattices and paints them on the black to cyan ramp.
pub const FRAGMENT: &str = include_str!("moire.wgsl");
/// The number of floats the fragment shader's uniform block expects.
...
/// The moire app: one corner rule laid over itself at a shifted count and quarter turn, on a square or hex grid.
pub struct Moire {
    set: Set,
}
...
impl Moire {
    /// Builds the app at offset four, angle ninety and the square lattice.
    pub fn new() -> Moire {
        Moire { set: Set::new() }
    }
    ...
    fn actions(&self, _iden: &Iden) -> Vec<Verb> {
        vec![
            Verb::new(
                ...
                json!({
                    "key": {
                        "offset": format!("int {OFFSET_MIN}..{OFFSET_MAX}"),
                        "angle": ANGLES.map(|a| a.to_string()).join(" | "),
                        "lattice": LATTICES.join(" | "),
                    },
                    "value": "of key",
                }),
            ),
