...
/// The picture skins the questions draw with.
pub mod skin;
...
/// The quiz app: one run, its standing, and the picture and names on show.
pub struct Quiz {
    set: Set,
    rng: Rng,
    seed: u64,
...
impl Quiz {
    /// Starts a ten-question run on seed zero with the first question rolled.
    pub fn new() -> Quiz {
        let mut quiz = Quiz {
            set: Set::new(),
            rng: Rng::new(0),
        ...
        out.push(Verb::new(
            ...
            json!({
                "key": {
                    ...
                    "skin": VARIANTS.join(" | "),
                    "reward_right": "int 0..10000",
                    "reward_wrong": "int -10000..0",
                },
                "value": "of key",
            }),
        ));
...
mod tests {
    ...
    fn correct_answer_scores_and_rerolls() {
        ...
        assert_eq!(out.effects[0].data, audio::cue("good"));
        assert_eq!(q.score, 1);
        assert!(!q.over);
    }
    #[test]
    fn wrong_answer_ends_the_round_honestly() {
        ...
        assert_eq!(out.effects[0].data, audio::cue("bad"));
        assert!(q.over);
        assert!(!q.won);
    ...
    fn state_carries_the_cells_fact() {
        ...
        let frame = mrlyskin::raster(crate::skin::corpus(), cells, 8, false).expect("a frame");
