...
/// The tile skin: one pen per kind, then the same kinds again in the crush pen.
pub mod skin;

use crate::skin::KINDS;
use mrlycore::audio;
...
impl Set {
    ...
    fn apply(&mut self, key: &str, value: &Json) -> Result<Json, &'static str> {
        match key {
            ...
            "reward_crush" => int(&mut self.reward_crush, value, (0, 10000)),
...
/// The crush app: a seeded board with one tile falling into it at a time.
pub struct Crush {
    set: Set,
    rng: Rng,
    seed: u64,
...
impl Crush {
    /// Starts a 9x9 round on seed zero with the first tile already spawned.
    pub fn new() -> Crush {
        let mut crush = Crush {
            set: Set::new(),
            rng: Rng::new(0),
...
impl App for Crush {
    ...
    fn manifest(&self) -> Manifest {
        Manifest::new("crush")
            .emoji("🍬")
            .category("games")
            .key("up", Call::new("crush.crush", json!({})))
            .key("down", Call::new("crush.drop", json!({})))
    ...
    fn call(&mut self, _iden: &Iden, call: &Call) -> Outcome {
        match call.verb.as_str() {
            "crush.move" => {
                ...
                if self.over {
                    out = out.emit(Effect::new("sound", audio::cue("lose")));
                }
                out
            }
            "crush.drop" => {
    ...
    fn save(&self) -> Json {
        json!({
            ...
            "crushable": self.crushable.clone(),
...
mod tests {
    ...
    fn drop_locks_the_piece() {
        ...
        assert!(out
            .effects
            .contains(&Effect::new("sound", audio::cue("blip"))));
    ...
    fn crush_clears_and_scores() {
        ...
        assert!(out
            .effects
            .contains(&Effect::new("sound", audio::cue("good"))));
    ...
    fn crushable_cells_wear_the_crush_roles() {
        ...
        c.mark_matches();
        let ids = c.ids();
        assert_eq!(ids[r as usize][0], KINDS as u8 + 2);
        assert_eq!(ids[r as usize][1], KINDS as u8 + 2);
    }
}
