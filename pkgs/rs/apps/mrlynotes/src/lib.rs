...
/// One stored note: an identifier and the text it carries.
pub struct Note {
    /// The note's identifier, never handed out twice.
    pub id: u64,
    /// The note's text, trimmed and never empty.
    pub text: String,
}

/// The notepad: the stored notes, the next free id and the live search query.
pub struct Notes {
    items: Vec<Note>,
    next: u64,
    query: String,
...
impl Notes {
    /// Builds an empty notepad whose first note takes id one.
    pub fn new() -> Notes {
        Notes {
            items: Vec::new(),
            next: 1,
    ...
    /// Appends a trimmed note and hands back its id, or nothing when the text is blank.
    ...
    /// Rewrites one note's text in place, refusing blank text and unknown ids.
    ...
    /// Deletes the note with this id and reports whether one was there.
    ...
    /// Returns the note with this id, if the list still holds one.
    pub fn get(&self, id: u64) -> Option<&Note> {
        self.items.iter().find(|n| n.id == id)
    }
    /// Returns every note in the order it was added.
    pub fn all(&self) -> &[Note] {
        &self.items
    }
    /// Sets the query the list is filtered by, trimmed and lowercased.
    ...
    /// Returns the notes whose text holds the query, or all of them when it is empty.
    ...
    /// Returns how many notes are stored, whatever the query.
    pub fn count(&self) -> usize {
        self.items.len()
    }
    /// Drops every note at once, leaving the next id where it stood.
