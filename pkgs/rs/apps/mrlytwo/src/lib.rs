...
/// The sixteen-pen tile skin the grid is drawn with.
pub mod skin;
...
impl Set {
    ...
    fn fits(number: i64, level: i64) -> bool {
        tile::size(number, level).is_some_and(|side| side <= MAX_SIDE)
    }
...
/// The tiling bench: one of five designs, its cells per side, its depth, and the colors of fill and void.
pub struct Two {
    set: Set,
}
...
impl Two {
    /// Opens the bench on a 5 by 5 carpet two levels deep, red fill on black void.
    pub fn new() -> Two {
        Two { set: Set::new() }
    }
    fn cell(&self) -> Cell2d {
...
mod tests {
    ...
    fn a_wrapped_level_never_reaches_the_grid() {
        ...
        assert_eq!(t.set.level, 2);
        t.load(&json!({ "number": 5, "level": 4294967298i64 }));
        assert_eq!(t.set.level, 2);
    }
    #[test]
