...
/// The colors app: the palette open at one page, beside a library of kept names.
pub struct Colors {
    index: usize,
    library: Vec<String>,
}
...
impl Colors {
    /// Opens the browser on the first color with every color in the library.
    pub fn new() -> Colors {
        Colors {
            index: 0,
            library: seed(),
...
impl App for Colors {
    ...
    fn actions(&self, _iden: &Iden) -> Vec<Verb> {
        vec![
            ...
            Verb::new(
                "colors.set",
                json!({ "key": { "name": NAMES.join(" | ") }, "value": "of key" }),
            ),
