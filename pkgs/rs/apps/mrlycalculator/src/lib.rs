...
/// One of the four pocket-calculator operations.
#[derive(Clone, Copy, PartialEq, Debug)]
pub enum Op {
    /// The sum of the two numbers.
    Add,
    /// The second number taken from the first.
    Sub,
    /// The product of the two numbers.
    Mul,
    /// The first number over the second, refused when the second is zero.
    Div,
}
...
/// The four-function calculator: a readout and one pending operation.
...
impl Calculator {
    /// Starts a cleared calculator showing 0.
    ...
    /// Returns the text on the readout.
    pub fn display(&self) -> &str {
        &self.display
    }
    /// Enters one digit, starting a fresh number when one is awaited.
    ...
    /// Enters the decimal point, at most one per number.
    pub fn dot(&mut self) {
        if self.waiting {
    ...
    /// Chooses an operator, first settling a pending one that has its second number.
    ...
    /// Settles the pending operation and shows the result.
    ...
    /// Clears the readout and everything held behind it.
    ...
    /// Flips the sign of the shown value.
