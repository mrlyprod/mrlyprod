...
/// The lettered skin the rain is drawn with: the whole alphabet once per fade band.
pub mod skin;

use crate::skin::{ALPHABET, BANDS};
use mrlycore::colors::{hex, hex_of, NAMES};
...
/// One seeded storm: the field of columns, how far each head has fallen and how each glyph is aging.
pub struct Matrix {
    set: Set,
    rng: Rng,
    seed: u64,
...
impl Matrix {
    /// Starts a 32 by 24 field pouring on seed zero.
    pub fn new() -> Matrix {
...
impl App for Matrix {
    ...
    fn actions(&self, _iden: &Iden) -> Vec<Verb> {
        ...
        for i in 0..self.set.palette.len() {
            keys[format!("palette.{i}")] = json!(&names);
        }
        vec![
            Verb::new("matrix.step", json!({ "n": "int" })),
            Verb::new("matrix.reset", json!({ "seed": "int" })),
            Verb::new("matrix.set", json!({ "key": keys, "value": "of key" })),
        ]
    }
