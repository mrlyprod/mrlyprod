mod tests {
    ...
    fn frame_carries_the_open_routes_state() {
        ...
        let mut os = Os::new(Iden::new("aria"))
            .install(Box::new(Notes::new()))
            .install(Box::new(mrlymenu::Menu::new()));
    ...
    fn settings_set_validates_in_app() {
        let mut os = Os::new(Iden::new("aria"))
            .install(Box::new(Home))
            .install(Box::new(mrlysettings::Settings::new()));
    ...
    fn shot_lands_in_the_roll() {
        let mut os = Os::new(Iden::new("aria"))
            .install(Box::new(mrlytwo::Two::new()))
            .install(Box::new(mrlyphotos::Photos::new()));
        os.call(Call::new("nav.open", json!({ "app": "two" })));
        let cells = os.read("two/cells", None).unwrap();
        let image = mrlyweb::registry::raster("two", &cells, 8, false)
            .unwrap()
            .image()
            .to_json();
    ...
    fn shot_fails_without_an_image() {
        let mut os = Os::new(Iden::new("aria"))
            .install(Box::new(mrlycolors::Colors::new()))
            .install(Box::new(mrlyphotos::Photos::new()));
    ...
    fn shot_on_photos_fails() {
        let mut os = Os::new(Iden::new("aria")).install(Box::new(mrlyphotos::Photos::new()));
        let out = os.call(Call::new("sys.shot", json!({})));
        assert!(!out.ok);
    ...
    fn file_effect_lands_in_files() {
        let mut os = Os::new(Iden::new("aria"))
            .install(Box::new(mrlycolors::Colors::new()))
            .install(Box::new(mrlyfiles::Files::new()));
    ...
    fn file_effect_without_files_app_is_dropped() {
        let mut os = Os::new(Iden::new("aria")).install(Box::new(mrlycolors::Colors::new()));
        os.call(Call::new("nav.open", json!({ "app": "colors" })));
    ...
    fn the_world_wears_the_ring() {
        let mut os = Os::new(Iden::new("aria"))
            .install(Box::new(mrlylog::Log::new()))
            .install(Box::new(Home));
        os.call(Call::new("nav.open", json!({ "app": "home" })));
    ...
    fn shared_facts_reach_the_worn_app() {
        let mut os = Os::new(Iden::new("aria"))
            .install(Box::new(mrlysettings::Settings::new()))
            .install(Box::new(mrlystudio::Studio::new()));
        os.call(Call::new(
            "settings.set",
