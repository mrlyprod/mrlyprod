...
fn shoot(os: &Os, app: &str) -> Json {
    ...
    mrlyweb::registry::raster(app, &cells, 8, false)
        .unwrap()
        .image()
        .to_json()
}
...
fn shader_sources_are_complete() {
    let all = mrlyweb::registry::programs();
    assert!(!all.is_empty());
    for program in all {
        let name = program.name;
        let source = mrlyweb::shaders::assemble(&program);
...
fn every_shade_resolves_a_program() {
    ...
    for app in mrlyweb::registry::catalogue() {
        ...
        let floats = mrlyweb::registry::floats(program)
...
#[test]
fn every_mesh_route_is_rigged() {
    let iden = Iden::new("guest");
    let rigged: Vec<&str> = mrlyweb::registry::rigs()
        .iter()
        .map(|rig| rig.route)
        .collect();
    ...
    for app in mrlyweb::registry::catalogue() {
        ...
        if app.state(&iden, None)["shade"]["program"] == json!("mesh") {
            assert!(rigged.contains(&route.as_str()), "{route} has no rig");
        }
        routes.push(route);
    }
    for route in rigged {
        assert!(
            ...
            "{route} is rigged and names no app"
        );
    }
}
...
fn every_cells_fact_resolves_a_skin() {
    let iden = Iden::new("guest");
    let corpus = mrlyweb::registry::corpus();
...
#[test]
fn paint_frame_is_golden() {
    let mut os = boot();
    os.call(Call::new("nav.open", json!({ "app": "paint" })));
    os.call(Call::new("paint.reset", json!({ "seed": 7 })));
    os.call(Call::new(
        "paint.stroke",
        json!({ "points": [[1, 1], [2, 2]] }),
    ));
    os.call(Call::new("paint.clear", json!({})));
    assert_eq!(os.envelope(None).to_json(), fixture("paint"));
}
...
#[test]
fn graph_frame_is_golden() {
    let mut os = boot();
    os.call(Call::new("nav.open", json!({ "app": "graph" })));
    os.call(Call::new(
        "graph.set",
        json!({ "key": "kind", "value": "tunnel" }),
    ));
    os.call(Call::new(
        "graph.set",
        json!({ "key": "fill", "value": "mint" }),
    ));
    os.call(Call::new(
        "graph.set",
        json!({ "key": "ghost", "value": true }),
    ));
    os.call(Call::new("graph.page", json!({ "dir": "next" })));
    assert_eq!(os.envelope(None).to_json(), fixture("graph"));
}
...
#[test]
fn fonts_frame_is_golden() {
    let mut os = boot();
    os.call(Call::new("nav.open", json!({ "app": "fonts" })));
    os.call(Call::new("fonts.pick", json!({ "char": "a" })));
    os.call(Call::new("fonts.scramble", json!({})).at(7));
    os.call(Call::new("fonts.tick", json!({})));
    os.call(Call::new("fonts.tick", json!({})));
    assert_eq!(os.envelope(None).to_json(), fixture("fonts"));
}
...
#[test]
fn emojis_frame_is_golden() {
    let mut os = boot();
    os.call(Call::new("nav.open", json!({ "app": "emojis" })));
    os.call(Call::new(
        "emojis.set",
        json!({ "key": "category", "value": "food" }),
    ));
    assert_eq!(os.envelope(None).to_json(), fixture("emojis"));
}

#[test]
fn studio_frame_is_golden() {
    let mut os = boot();
    os.call(Call::new("nav.open", json!({ "app": "studio" })));
    os.call(Call::new("studio.press", json!({ "midi": 43 })));
    os.call(Call::new("studio.press", json!({ "midi": 55 })));
    os.call(Call::new("studio.lift", json!({ "midi": 43 })));
    assert_eq!(os.envelope(None).to_json(), fixture("studio"));
}
