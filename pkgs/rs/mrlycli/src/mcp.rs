...
/// Serves the Mrly world to an MCP client over stdio.
pub fn serve() {
...
fn schema(args: &Json) -> Json {
    ...
    if let Some(fields) = args.as_object() {
        let picked = menu(fields);
        for (key, hint) in fields {
            let shaped = match &picked {
                Some((_, of)) if key == of => json!({}),
                Some((target, _)) if key == target => chooser(hint),
                _ => field(hint),
            };
            properties.insert(key.clone(), shaped);
        }
    }
    properties.insert(
        "now".to_string(),
...
fn chooser(hint: &Json) -> Json {
    let names: Vec<Json> = match hint.as_object() {
        Some(fields) => fields.keys().map(|key| json!(key)).collect(),
        None => Vec::new(),
    };
    json!({
        "type": "string",
        "enum": Json::Arr(names),
        "description": crate::shape(hint, ": "),
    })
}

fn nested(hint: &Json) -> Json {
    let Some(fields) = hint.as_object() else {
        return json!({ "description": hint.to_string() });
    };
    let mut properties = Map::new();
    for (key, inner) in fields {
        properties.insert(key.clone(), field(inner));
    }
    json!({
        "type": "object",
        "properties": Json::Obj(properties),
        "required": Json::Arr(fields.keys().map(|key| json!(key)).collect()),
    })
}
...
fn field(hint: &Json) -> Json {
    ...
    if token.contains(" | ") {
        ...
        if options.iter().all(|option| whole(option)) {
            let numbers: Vec<Json> = options
                .iter()
                .filter_map(|o| o.parse::<i64>().ok())
                .map(Json::Int)
                .collect();
            return json!({ "type": "integer", "enum": Json::Arr(numbers), "description": token });
        }
    ...
    match token {
        ...
        _ if token.starts_with("points ") => json!({
            "type": "array",
