...
fn usage() {
    eprintln!("mrly - a terminal face for the mrly kernel");
    eprintln!();
    eprintln!("usage:");
    eprintln!("  mrly               tui on a terminal, repl on a pipe (default)");
    eprintln!("  mrly tui           raw-mode terminal face");
    eprintln!("  mrly repl          line-by-line interactive session");
    eprintln!(
        "  mrly mcp           an MCP server over stdio: tools/list, tools/call, resources/read"
    );
    eprintln!(
        "  mrly run [file]    replay a call script, print the envelope (--facts trims grids)"
    );
    eprintln!("  mrly render [file] draw the final frame as colored blocks");
    eprintln!("  mrly shot [file]   write the final frame as a PNG (--out path, --size N)");
    eprintln!("  mrly shot --route <name> [--size N]");
    eprintln!("                     render one route from a fresh boot");
    eprintln!("  mrly shot --list   print every route the eye answers for");
    eprintln!("  mrly shot --record re-pin every shot golden in tests/shots");
    eprintln!("  mrly read <app[/path]> [file] [shape]");
    eprintln!("                     print an app's view, one field, or a shaped subtree");
    eprintln!("  mrly watch <app/path> [file]");
    eprintln!("                     poll the field and print it when it changes");
    eprintln!("  mrly goose <app> [--seed N] [--steps K] [--trace] [--read path]");
    eprintln!("                     drive an app with random legal calls");
    eprintln!("  mrly drive [file]  play a wire screenplay: open, call, assert");
    eprintln!("  mrly frame [file] [WxH]");
    eprintln!("                     replay a screenplay and print the TUI screen as text");
    eprintln!("  mrly frame --record");
    eprintln!("                     re-pin every frame golden in tests/frames");
    eprintln!("  mrly list          print the kernel surface as JSON");
    eprintln!("  mrly verbs [app]   list apps, or one app's verbs and args");
    eprintln!("  mrly help          show this message");
    eprintln!();
    eprintln!("repl:");
...
fn shape(hint: &Json, gap: &str) -> String {
    match hint {
        Json::Str(token) => token.clone(),
        Json::Obj(fields) => {
            let inner: Vec<String> = fields
                .iter()
                .map(|(k, v)| format!("{k}{gap}{}", shape(v, gap)))
                .collect();
            format!("{{{}}}", inner.join(", "))
        }
        _ => "?".to_string(),
...
fn snap(os: &Os, app: &str, size: usize) -> Result<Vec<u8>, &'static str> {
    let pixels = mrlyweb::eye::frame(os, app, size, size).ok_or("nothing to shoot here")?;
    mrlycore::png(&pixels, size, size, 1).map_err(|_| "could not render frame")
}

fn brush() -> Json {
    let mut points = Vec::new();
    for x in 6..=17 {
        points.push(json!([x, 6]));
    }
    for y in 7..=17 {
        points.push(json!([6, y]));
    }
    for x in 7..=17 {
        points.push(json!([x, 17]));
    }
    for y in (7..=16).rev() {
        points.push(json!([17, y]));
    }
    for y in 10..=13 {
        for x in 10..=13 {
...
fn wake(route: &str) -> Os {
    let mut os = build();
    os.call(Call::new("nav.open", json!({ "app": route })));
    match route {
        "mandelbrot" | "julia" | "matrix" => {
            os.call(Call::new(&format!("{route}.step"), json!({ "n": 24 })));
        }
        "bang" => {
            os.call(Call::new("bang.page", json!({ "dir": "next" })));
        }
        "crush" => {
            os.call(Call::new("crush.step", json!({ "n": 600 })));
        }
        "life" => {
            os.call(Call::new("life.reset", json!({ "pattern": "soup" })));
        }
        "mines" => {
            os.call(Call::new("mines.reveal", json!({ "cell": 0 })));
        }
        "dice" => {
            os.call(Call::new(
                "dice.set",
        ...
        "snake" => {
            os.call(Call::new(
                "snake.set",
                json!({ "key": "apples", "value": 16 }),
            ));
            os.call(Call::new("snake.step", json!({ "n": 40 })));
        }
        "ttt" => {
            os.call(Call::new(
                "ttt.set",
                json!({ "key": "opponent", "value": "off" }),
            ));
            for cell in [4, 0, 8] {
                os.call(Call::new("ttt.place", json!({ "cell": cell })));
            }
        }
        "paint" => {
...
fn shots(write: bool) -> usize {
    let home = format!("{}/tests/shots", env!("CARGO_MANIFEST_DIR"));
    if write {
    ...
    for app in mrlyweb::registry::catalogue() {
        ...
        let os = wake(&route);
        let Some(pixels) = mrlyweb::eye::frame(&os, &route, GOLDEN, GOLDEN) else {
            continue;
        };
        match write {
            true => {
                let Ok(bytes) = mrlycore::png(&pixels, GOLDEN, GOLDEN, 1) else {
                    continue;
                };
                if std::fs::write(format!("{home}/{route}.png"), bytes).is_err() {
                    continue;
                }
            }
            false => println!("{route}"),
...
fn shot(args: &[String]) {
    ...
    while let Some(a) = it.next() {
        match a.as_str() {
            ...
            "--size" => size = it.next().and_then(|n| n.parse().ok()).unwrap_or(size),
            "--route" => route = it.next().cloned(),
            other => path = Some(other),
        }
    }
    if list {
    ...
    if record {
        println!("{} shots", shots(true));
        return;
    }
    let (os, app) = match route {
        Some(name) => (wake(&name), name),
        None => {
            let os = replay(path);
            let app = os.envelope(None).route.map(|r| r.app).unwrap_or_default();
            (os, app)
...
fn paint(env: &Json) -> String {
    ...
    let grid = state
        .get("cells")
        .and_then(|cells| mrlyweb::registry::raster(app, cells, 4, true))
        .map(|f| f.fact())
        .unwrap_or(Json::Null);
