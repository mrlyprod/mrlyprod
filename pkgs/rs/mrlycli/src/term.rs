...
/// A terminal held in raw mode until the guard is dropped.
pub struct Raw {
    saved: libc::termios,
}

impl Raw {
    /// Switches the terminal into raw mode and returns the guard that restores it.
    pub fn enter() -> Raw {
...
/// Reports the terminal size in columns and rows, falling back to eighty by twenty-four.
...
/// Reports whether both input and output are attached to a terminal.
pub fn tty() -> bool {
...
pub enum Key {
    ...
    /// The escape key.
    Esc,
    /// A character held with control.
    Ctrl(char),
}

/// Decodes a run of terminal bytes into keypresses.
...
/// One character position on the screen with its colours.
pub struct Cell {
    /// The character drawn in the position.
    pub ch: char,
    /// The foreground colour, or the terminal default.
    pub fg: Option<(u8, u8, u8)>,
    /// The background colour, or the terminal default.
    pub bg: Option<(u8, u8, u8)>,
    /// Whether the position is drawn faintly.
    pub dim: bool,
}
...
impl Cell {
    /// Builds a cell carrying a character in a foreground colour.
    ...
    /// Builds a half-block cell that paints two stacked pixels.
...
/// Reports whether a character occupies two columns.
...
/// A grid of cells holding one frame.
pub struct Screen {
...
impl Screen {
    ...
    /// Writes one cell, ignoring positions beyond the grid.
    pub fn put(&mut self, x: usize, y: usize, cell: Cell) {
        if x < self.w && y < self.h {
            self.cells[y * self.w + x] = cell;
        }
    }
    /// Draws a string from a position and reports the column it ended on.
    pub fn text(
        &mut self,
        x: usize,
        y: usize,
...
/// Renders the escape sequence that turns one frame into the next.
...
impl Screen {
    /// Renders the screen as plain text.
