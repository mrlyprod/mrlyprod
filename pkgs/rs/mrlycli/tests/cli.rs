...
#[test]
fn every_shot_matches_its_golden() {
    let root = std::path::Path::new(env!("CARGO_MANIFEST_DIR"));
    let home = root.join("tests/shots");
    let listed = String::from_utf8(piped(&["shot", "--list"], "")).unwrap();
    let mut routes: Vec<String> = listed.lines().map(str::to_string).collect();
    routes.sort();
    let mut pinned: Vec<String> = std::fs::read_dir(&home)
        .unwrap()
        .map(|entry| {
            entry
                .unwrap()
    ...
    assert_eq!(
        routes, pinned,
        "the goldens are not the set the eye answers for"
    );
    for route in &routes {
        let out =
            std::env::temp_dir().join(format!("mrly_shot_{route}_{}.png", std::process::id()));
        let status = mrlycli()
            .args([
                "shot",
                "--route",
        ...
        let golden = std::fs::read(home.join(format!("{route}.png"))).unwrap();
        assert!(
            fresh == golden,
            "{route} drifted; re-pin with test.py record"
        );
    }
}

#[test]
fn no_golden_is_a_blank_screen() {
    let home = std::path::Path::new(env!("CARGO_MANIFEST_DIR")).join("tests/shots");
    let mut count = 0;
    for entry in std::fs::read_dir(&home).unwrap() {
        let path = entry.unwrap().path();
        count += 1;
        let route = path.file_stem().unwrap().to_string_lossy().to_string();
        let (width, height, pixels) = mrlycore::unpng(&std::fs::read(&path).unwrap()).unwrap();
        let mut tally: std::collections::HashMap<[u8; 4], usize> = std::collections::HashMap::new();
        for pixel in &pixels {
            *tally.entry(*pixel).or_default() += 1;
        }
        let ink = pixels.len() - tally.values().copied().max().unwrap_or(0);
        assert!(
            ink * 100 >= width * height,
            "{route} inks {:.2}% of its frame; live the screen in wake, then test.py record",
            ink as f64 * 100.0 / (width * height) as f64
        );
    }
    assert!(count >= 2, "there are no goldens to weigh");
}
