...
/// The tile configuration in three dimensions.
...
/// Draws a tile from the config with random cube orientations.
pub fn create(config: &Config) -> Result<Tile> {
    spec::create(config, rotation)
}

/// Draws a random tile up to the given size.
