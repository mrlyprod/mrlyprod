...
use crate::dim::serializer::{byte_cube, color_grid, count_cube, parse, tag_layer, types_field};
...
pub fn to_json(cell: &Cell3d) -> String {
    ...
    let nest = |flat: &dyn Fn(usize) -> Json| -> Json {
        (0..shape[0])
            .map(|i| {
                (0..shape[1])
                    .map(|j| {
                        (0..shape[2])
                            .map(|k| flat((i * shape[1] + j) * shape[2] + k))
                            .collect::<Json>()
                    })
                    .collect::<Json>()
            })
    ...
    if let Some(colors) = &cell.cell.colors {
        data["colors"] = nest(&|at| colors[at].into());
    }
    if let Some(tags) = &cell.cell.tags {
        data["tags"] = nest(&|at| tags.at(at).into());
    }
    data.to_string()
}
