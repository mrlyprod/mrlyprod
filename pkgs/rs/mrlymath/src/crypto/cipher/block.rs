...
/// The shape of the Feistel cipher.
#[derive(Clone, Copy, Debug)]
pub struct Config {
    /// The side length of the square half-block grid.
    pub side: usize,
    /// The Feistel rounds per block.
    pub rounds: usize,
}
...
impl Config {
    /// Returns the half-block size in bits.
...
/// The keyed block cipher, ready to encrypt and decrypt.
pub struct Cipher {
    config: Config,
    keys: Vec<Vec<u8>>,
}
...
impl Cipher {
    /// Builds a cipher by expanding the key into one round key per round, or an error when blocks are not whole bytes.
...
fn unpad(data: &[u8]) -> Result<Vec<u8>> {
    let pad_len = *data
        .last()
        .ok_or_else(|| mrlycore::MrlyError::Value("cannot unpad empty data.".into()))?
        as usize;
...
/// Encrypts the padded message block by block in CBC mode, returning the ciphertext bytes.
...
/// Decrypts a ciphertext back to the message, or an error on a bad length or broken padding.
