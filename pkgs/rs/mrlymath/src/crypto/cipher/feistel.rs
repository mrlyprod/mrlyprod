...
/// Mixes a half-block with its round key through three permutation rounds.
...
/// Runs the block's two halves forward through one Feistel round per key.
...
/// Runs the block's two halves backward through the keys, undoing encrypt_block.
...
/// The two halves of a block after a Feistel round.
#[derive(Clone, Debug)]
pub struct RoundState {
    /// The rounds applied so far.
    pub round: usize,
    /// The left half bits.
    pub left: Vec<u8>,
    /// The right half bits.
...
/// Returns the halves after every round, starting from the untouched block.
