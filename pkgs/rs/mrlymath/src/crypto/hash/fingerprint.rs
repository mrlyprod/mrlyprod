...
/// Builds a side-by-side identicon grid from the digest bits, mirrored left to right.
...
/// Renders the digest's identicon to PNG bytes at the given scale, inked in a hue the digest picks.
pub fn fingerprint(digest: &Digest, side: usize, scale: usize) -> Result<Vec<u8>> {
    ...
    let colors = mrlycore::io::colorize(&grid, &colorizer, 1);
    mrlycore::io::png(&colors, side, side, scale)
}
