...
///
/// ```
/// let mut net = mrlymath::graph::Network::new(2);
/// net.add_node(vec![0.0, 0.0]).unwrap();
/// net.add_node(vec![1.0, 0.0]).unwrap();
/// net.add_branch(0, 1, 1.0).unwrap();
/// assert_eq!(mrlymath::graph::roles(&net), vec![mrlymath::graph::Role::Tip; 2]);
/// ```
...
/// assert_eq!(mrlymath::graph::census(&net).components, 2);
/// ```
