...
impl Sequence {
    /// Returns the sequence's parseable name.
    ...
    /// Returns every fixed sequence, code families excluded.
...
fn parse_code(s: &str) -> Result<u128> {
    s.trim().parse::<u128>().map_err(|_| {
        mrlycore::MrlyError::Value(format!("invalid code {s:?} (expected an integer)."))
    })
}
