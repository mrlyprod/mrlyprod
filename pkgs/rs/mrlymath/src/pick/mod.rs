...
/// The sixteen design names, blank through solid.
pub const NAMES: [&str; 16] = [
    "blank", "dot", "dash", "slash", "tick", "bar", "grid", "comb", "node", "step", "weave",
    "mesh", "wave", "loom", "quilt", "solid",
];

/// Returns the first count design codes, skipping blank and solid.
pub fn vocab(count: usize) -> Vec<usize> {
    (1..NAMES.len() - 1).take(count).collect()
}

/// Returns the name of a design code, or blank when out of range.
pub fn name(code: usize) -> &'static str {
    NAMES.get(code).copied().unwrap_or("blank")
}

/// Renders a design code as a square cell of the given size, its marks in the foreground color over the background.
