...
- **census**, **graph**, **boolean**, and **fft** measure what the rest makes.
- **wave** rings designs as acoustic media; **lattice** counts their visible points.
- **formulas** counts in closed form; **prime** reads a number as rectangles, pairs, and squares.
- **factor** splits that same number into primes, divisors, and Mobius values; **series** sums zeta and pins the Bernoulli fractions.
- **crypto** builds block ciphers and sponge hashes; **space** holds vectors, meshes, and packs.
- **play** hides that same arithmetic in pictures: number spirals, Pascal triangles, and Collatz staircases.

## Previews

- `cargo run -p mrlymath --example paints` prints one painted tile per edition as JSON.
- `uv run python utils/paints.py` draws those tiles into `data/paints`.
