/// Fills a rectangle of the pixel buffer with one color, clipped to the edges.
...
/// Paints the on bits of sprite rows into the pixel buffer at a scale, clipped to the edges.
#[allow(clippy::too_many_arguments)]
pub fn blit(
    buf: &mut [[u8; 4]],
    w: usize,
...
/// Rasters the text at the first listed scale that fits the field, trimming trailing characters at the last scale when none does.
///
/// ```
/// let (_, scale, text) = mrlyskin::draw::fit("hi", 1000, &[3, 2]);
/// assert_eq!((scale, text.as_str()), (3, "hi"));
/// ```
