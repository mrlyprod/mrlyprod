...
/// One deterministic world of installed apps, with a focus, a clock and a journal ring.
pub struct Os {
    iden: Iden,
...
impl Os {
    /// Builds an empty world for the given identity, opened on the menu.
    ...
    /// Installs an app and returns the world, focusing the first app installed.
    ...
    /// Focuses the named app, or refuses when it is not installed.
    ///
    /// ```
    /// use mrlyos::{Iden, Os};
    /// let mut os = Os::new(Iden::new("aria"));
    /// assert_eq!(os.open("ghost"), Err("no such app"));
    /// ```
    ...
    /// Journals one call, routes it to the system or an app, settles its effects, and returns the outcome.
    ...
    /// Builds the envelope of the present tick, with the focused view pruned to the shape.
    ...
    /// Reads an app's geometry, uniforms or tris buffer as floats, without the base64 the wire needs.
    pub fn floats(&self, path: &str) -> Option<Vec<f32>> {
        let (app, path) = split(path);
        let i = self.find(&app)?;
        let [head] = path.as_slice() else {
            return None;
        };
        match head.as_str() {
            "geometry" => self.apps[i].geometry(),
            "uniforms" => self.apps[i].uniforms(),
            "tris" => self.apps[i].tris(),
            _ => None,
        }
    }
    /// Lists the version, the apps, every app's verbs and the nav verbs, pruned to the shape.
    ...
    /// Returns the route of every installed app.
...
mod tests {
    ...
    #[test]
    fn floats_read_the_reserved_heads_unwrapped() {
        let os = boot();
        let geometry = os.floats("toy/geometry").unwrap();
        assert_eq!(geometry.len(), 2);
        assert_eq!(geometry, vec![1.0, -2.0]);
        assert_eq!(os.floats("toy/tris"), Some(vec![0.5]));
        assert_eq!(os.floats("toy/uniforms"), None);
        assert_eq!(os.floats("toy/nest"), None);
        assert_eq!(os.floats("toy/geometry/0"), None);
        assert_eq!(os.floats("toy"), None);
        assert_eq!(os.floats("ghost/geometry"), None);
        assert_eq!(os.floats("bare/geometry"), None);
    }
