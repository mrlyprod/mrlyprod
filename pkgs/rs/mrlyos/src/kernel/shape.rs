...
/// Prunes the state to the keys an object shape names, recursively, copying whole whatever a non-object shape touches.
///
/// ```
/// use mrlycore::json;
/// let state = json!({ "a": { "x": 1, "y": 2 }, "b": 3 });
/// assert_eq!(mrlyos::kernel::prune(&state, &json!({ "a": { "x": 1 } })), json!({ "a": { "x": 1 } }));
/// ```
