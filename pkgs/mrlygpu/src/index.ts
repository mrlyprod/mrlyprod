...
export type Feeds = {
  geometry?: (route: string) => Float32Array | undefined
...
export function init(shaders: Shaders, hooks: Feeds = {}): void {
  sources = shaders
  feeds = hooks
  if (booted) return
  booted = true
...
function ensureShape(slot: Slot, shade: Shade): Shape | null {
  ...
  if (feeds.geometry === undefined || shade.route === undefined || shade.mesh === undefined) return null
  const packed = feeds.geometry(shade.route)
...
function paintMesh(canvas: HTMLCanvasElement, slot: Slot, shade: Shade, sent: number[]): void {
  const pipes = mesh()
  if (pipes === null || device === null) return
  const { width, height } = fit(canvas)
  const data = Float32Array.from(slot.tween !== null ? blend(slot, performance.now()) : sent)
...
export function pull(shade: Shade): number[] | null {
  if (feeds.uniforms === undefined) return null
  const packed = feeds.uniforms(shade.route ?? shade.program)
  return packed === undefined ? null : Array.from(packed)
}

export function draw(canvas: HTMLCanvasElement, node: Scene, next: number[] | null): boolean {
  if (device === null || node.shade === undefined || next === null) return false
...
export function drop(canvas: HTMLCanvasElement): void {
...
function commit(canvas: HTMLCanvasElement, slot: Slot, node: Scene, next: number[]): void {
  ...
  const smooth =
    ...
    !jumps(spec, prev, next)
  if (smooth) {
    const span = Math.min(Math.max(now - slot.stamp, BEAT), LINGER)
    slot.tween = { prev: prev as number[], next, from: now, span, spec: spec as Spec }
...
function paint(canvas: HTMLCanvasElement, slot: Slot): void {
  const shade = slot.node.shade
  const sent = slot.sent
  if (device === null || shade === undefined || sent === null) return
  if (shade.program === "mesh") {
    paintMesh(canvas, slot, shade, sent)
    return
  }
