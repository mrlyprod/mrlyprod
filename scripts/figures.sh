...
case "${1:-}" in
  check) cargo check -q -p figures --bins -p mrlyfig --tests; exit 0 ;;
...
BENCH_TMP="$(mktemp "${TMPDIR:-/tmp}/mrlyfig-bench.XXXXXX")"
export BENCH_TMP
...
c=0
MRLYFIG_THEME="$0" perl -e "alarm 5; exec @ARGV; exit 127" "target/fig/$1" || c=$?
