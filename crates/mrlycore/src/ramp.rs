...
#[derive(Clone, Debug)]
pub enum Colorizer {
    /// The background at zero, the foreground for everything else.
    TwoTone {
        /// The color at zero.
        background: Color,
        /// The color everywhere else.
        foreground: Color,
    },
    /// The background at zero, then the palette cycled by value.
    Wrap {
        /// The color at zero.
        background: Color,
        /// The colors cycled by value.
        palette: Vec<Color>,
    },
    ...
    /// A lookup table by exact value, with one color for the unlisted.
    Exact {
        /// The color per exact value.
        table: HashMap<usize, Color>,
        /// The color for values off the table.
        unlisted: Color,
    },
}
...
impl Colorizer {
    ...
    /// Builds a colorizer with one background and one foreground.
    ...
    /// Builds a colorizer that cycles the palette for values above zero.
    ...
    /// Builds an exact-table colorizer that leaves unlisted values transparent.
    pub fn exact(entries: &[(Color, Vec<usize>)]) -> Result<Colorizer> {
        Colorizer::exact_with(entries, ALPHA)
    }
    /// Builds an exact-table colorizer, or an error on a duplicate color or tag.
