...
impl Design {
    ...
    ///
    /// ```
    /// let design = mrlynum::ladder::Design::new(3, &[0, 1]).unwrap();
    ...
    ///
    /// ```
    /// let shallow = mrlynum::ladder::Design::with_peel(2, &[0, 1], 4).unwrap();
...
///
/// ```
/// let design = mrlynum::ladder::Design::new(2, &[0, 1]).unwrap();
/// let s = mrlynum::zeta::Complex::new(2.0, 0.0);
/// let (value, bound) = mrlynum::ladder::zeta(&design, s, 1e-10).unwrap();
