...
- `research-hexagon`: the carpet cut stack at odd sides 3 to 55 read on the hexagon's own mesh of side 110 and painted as excess over the background, the hexagram, the quarter-line bands, the fading centre star; hex; carpet, odd n 3..55, mesh 110; blue through panel to gold
...
- `paper-sparse-mertens-under-grh`: the level-3 digit transform of a missing-digit set, all 343 frequencies of `|hat F_3(a/343)|` mirrored, the `l^1` mass the proof pays; plot; q = 7, digit 3 removed, stems at the per-digit root of the mass; blue to gold on the ground
...
- `demo-formulas`: the page's eight systems as eight curves of their normalised gap on log-log axes: five constant chasers falling in parallel, pi(n) against li humped, the 1/g(2n) comet, |M(n)|/sqrt(n) flat; plot; n 2..2000, 4.5 decades; blue + gold + orange
- `demo-wallis`: the plane Wallis sieve at level 3, its 201 punctures as filled squares over the surviving body, one ink a level so the schedule reads as three sizes; grid; word 3, 5, 7, side 105 drawn 840 px, 9 percent margin; body line, holes gold, orange, blue
...
- `demo-modes`: the eigenvalue field of the level-5 base-3 carpet mask, 243 by 243 frequencies of |lambda(t)| / 8^5 read at its per-level root and centred on t = 0, the zero frequency at ratio 1 with the large values gold along the axes; field; code 495, side 3, level 5, 3 px a frequency; ground to blue to gold
- `demo-tube`: the level-5 carpet with its inner tube at eps = 21/729 lining every hole, and on the diagonal below the closed profile G(t) over one period with its 0.3662 percent swing exaggerated; plot; code 495, base 3, level 5, eps 7 cells, t in [1/3, 1); fg cells, blue tube, gold curve on ground
- `demo-weights`: the level-5 mass field of the base-3 carpet weighted 5,4,3,4,2,3,2,1 over 24, the two heaviest adjacent so the mass drifts to the top left at every scale, read at gamma 0.12; field; code 495, level 5, 243 by 243, 32768 cells; ground to blue to gold
- `demo-shell`: the circle of radius 242 crossing the level-5 carpet, its 485 crossed cells gold where the design keeps them and blue where it drops them, threaded through the 53, 17 and 5 nested boxes of levels 2, 3 and 4; grid; code 7, base 2, side 3, level 5, r 242; gold + blue, boxes dim at three weights, carpet line
- `demo-star`: the carpet's hexagonal cut layers at odd sides 1 to 55 stacked on the hexagon of the deepest, the six-armed star of the arms x = y, y = z, z = x lit over a quiet field of the same ink; field; code 23, base 2, 28 layers, band half-width 6 cells, 400 cells at 5 by 5 samples; blue and gold over line on ground
- `paper-first-base-below-a-quarter`: every one-missing-digit set of bases 6 to 40 as a dash at its certified l^1 exponent, dim above the quarter line and gold below, first gold at base 21, whole column gold from 34 on; plot; windows 3 to 5, sub-scan 4; gold + dim, line blue
- `demo-echo`: the power spectrum of the base-10 digit design missing the digit 9, its meter M_F(x)/x^(alpha/2) read uniformly in log x, standing against the first thirteen zeta ordinates and the design's own pole lattice; plot; base 10, digits 0 to 8, depth 5, 4096 samples, band 4 to 60; gold the zeta ordinates, pink the pole lattice, blue the spectrum
- `paper-design-dirichlet-inverse`: the base-3 F={0,1} pole comb on Re s = 0.630930 with three censused zeros right of it, one certified point boxed and two open bands, beside the integers' line at Re s = 1 and its empty half-plane; plot; q 3, k 2, 11 poles below Im 60, rho 0.720790 + 28.605680i, 2 bands; violet comb, gold zeros, half-plane in panel
- `research-zeta`: the zero set of a design zeta beside the integers', both combs of the pole lattice drawn as rings: base 3 {0,1} carrying gold live teeth off its abscissa and a blue second family, base 2 full set carrying no tooth at all, white cofactor-only zeros ON its abscissa and the blue critical line; plot; strip Re s in [alpha-1.2, alpha+0.7], Im s to 40, 13 and 10 zeros; blue family, gold teeth, white cofactor
