...
use mrlycore::tile::{Design, Group, Source, Tile as Recipe};
...
mod tests {
    ...
    fn built(recipe: &Recipe) -> crate::two::Cell2d {
        tile2d::build(recipe).unwrap()
    }
    ...
    fn seeded_tiles_round_trip() {
        ...
        for s in 0..300 {
            seed(s);
            let recipe = tile2d::create(&config).unwrap();
