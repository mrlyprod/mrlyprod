...
/// The names of the four built-in solids.
pub const SOLIDS: [&str; 4] = ["cube", "tetra", "octa", "icosa"];

const S3: f32 = 0.57735027;
const PHI: f32 = 1.618034;

/// A triangulated solid with one outward unit normal per face.
#[derive(Clone, Debug)]
pub struct Mesh {
    /// The vertex positions.
    pub verts: Vec<Vec3>,
    /// The faces as triples of vertex indices.
...
impl Mesh {
    /// Returns the vertex pairs where faces of different normals meet, skipping coplanar seams.
...
/// Builds the named solid, falling back to the cube for unknown names.
///
/// ```
/// let mesh = mrlymath::space::solid("icosa");
/// assert_eq!((mesh.verts.len(), mesh.faces.len()), (12, 20));
/// ```
...
/// Builds the cube, scaled to the unit ball.
pub fn cube() -> Mesh {
...
/// Builds the tetrahedron, scaled to the unit ball.
pub fn tetra() -> Mesh {
    let verts = vec![
...
/// Builds the octahedron, scaled to the unit ball.
pub fn octa() -> Mesh {
    let verts = vec![
...
/// Builds the icosahedron, scaled to the unit ball.
pub fn icosa() -> Mesh {
    let verts = vec![
