...
- **blend** wraps mrlylab and `mrlyrs::num`: registry sequences as terms, ratios, differences, recurrences and term operations.
...
- **ledger** wraps mrlylab and `mrlyrs::math`: every measure of every design as a sequence, searched, identified and spelled.
- **life** wraps `mrlyrs::life`: life grids stepped, run to a fate, and driven by named sequences.
- **magic** wraps mrlylab and `mrlyrs::math`: the folded design, its census, its press readings and its prefix rates.
...
- **prime** wraps mrlylab and `mrlyrs::num`: the sieve stepped, the stone pile, the count chart and the carpet witness.
...
- **spin** wraps mrlylab, `mrlyrs::math` and `mrlyrs::num`: designs, moire fields and slices spun about their centre into ring profiles and wheels.
...
- **volume** wraps mrlylab and `mrlyrs::math`: cube designs stacked into a moire volume, its faces at a level, and the planes that cut it.
