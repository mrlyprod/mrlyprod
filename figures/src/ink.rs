...
/// The theme every figure is painted in: light when FIGURES_THEME is "light", dark otherwise, read once.
pub fn theme() -> &'static Theme {
    static THEME: OnceLock<&'static Theme> = OnceLock::new();
    THEME.get_or_init(|| match std::env::var("FIGURES_THEME").as_deref() {
        Ok("light") => &LIGHT,
        _ => &DARK,
    })
}
