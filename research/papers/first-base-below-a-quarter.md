---
title: The First Base Below a Quarter
lead: Base 21 missing the digit 0 is the least base at which a one-missing-digit set has the `l^1` exponent of its digit transform below the bar `1/4`, all 108 sets of the bases 3 to 20 certified above it; 34 is the least base at which every digit clears and every base from 34 clears at every digit, from 125 by a proof; under the generalized Riemann hypothesis each clearance is a power saving for the Mobius sum over the set.
date: 2026-09-21
figure: paper-first-base-below-a-quarter
shelf: first-base-below-a-quarter
---

Fix a base and cross out one digit; the integers that can still be written form a thin set with no multiplicative structure, and everything analytic known about it passes through the `l^1` exponent `alpha_1` of its digit transform, how much of the transform survives when the whole grid of frequencies is added up in absolute value. A quarter is a different bar: under the generalized Riemann hypothesis, expanding the Mobius sum over the set in additive characters and paying the uniform bound `x^(3/4+eps)` at each frequency beats the set's own counting function exactly when `alpha_1 < 1/4`, and Theorem 5.1 proves that transfer, `|M_F(x)| << A_F(x)^(1 - delta + eps)` with `delta = (1/4 - alpha_1)/alpha`. Base `21` with the digit `0` removed is the least base at which a one-missing-digit set falls below the bar, `alpha_1 in [0.2499765, 0.2499771]`, since every one of the `108` distinct sets of the bases `3` to `20` is certified above it. Base `34` is the least base at which every excluded digit clears, and every base from `34` on does: its `17` sets and the `3663` sets of the bases `35` to `125` are certified, every base from `21` to `33` keeps a digit certified above the bar, the last being `33` at the digit `15`, and an elementary majorant that forgets the digit proves `alpha_1 < 1/4` from base `125` on. Every certified number is interval arithmetic with directed rounding, its generator named.
...
Three results say where the dashes cross. The first is a theorem: `alpha_1 < 1/4` for every base `q >= 125` and every excluded digit, with the explicit bound of Theorem 4.7, uniform in the digit because its first step throws the digit away: the triangle inequality leaves a majorant built from the Dirichlet kernel, its level product expands over subsets of the digit positions, every maximal run telescopes into one Dirichlet kernel at a higher modulus, and what remains is a sum of Lebesgue sums, each bounded by Lemma 4.4. The other two are computations by a transfer matrix on windows of digits and the test of Collatz and Wielandt: base `21` missing `0` below the bar and every base below `21` above it at every digit (Facts 6.1 and 6.2), base `34` below at every digit and every base from `21` to `33` above it at some digit (Fact 6.3), every set of the bases `35` to `125` below (Fact 6.4).
...
The last sentence is the test of Collatz (1942) and Wielandt (1950): a float power iteration supplies `y`, one pass in interval arithmetic turns it into a rigorous bound, and a poor vector weakens the bound and never breaks it. A cell containing a zero of `|hat F|` has `G^- = 0`, and near a zero a whole row of `M^-` can vanish, so the lower test runs on the restriction `M''` to the states where the float Perron vector exceeds `10^(-9)` of its maximum, which is the plain test wherever every state is live. Cell suprema and infima are enclosed by a sub-scan of `m` points plus the Lipschitz correction `Lambda/(2 m q^n)`, `Lambda = (2 pi/k) sum_(a in F) a`.
...
**Fact 6.1 (the first base).** Base `21` with the digit `0` removed satisfies `alpha_1 in [0.2499765, 0.2499771]` at `(6, 8)`, clearing `1/4` by more than `2.29 * 10^(-5)`, which is `1/4 - 0.2499771`; the same cell reads `[0.2499715, 0.2499821]` at `(5, 8)` and `[0.2498658, 0.2500871]` at `(4, 8)`, which straddles the bar, and a second implementation returns `[0.2499715, 0.2499822]` at `(5, 8)`. Generators: `lab/py/digit-transform-norms`, verbs `six` and `least`; `lab/py/mobius-region`, verb `params 21 123456789abcdefghijk`.

**Fact 6.2 (nothing below it).** Every one of the `108` distinct sets of the bases `3 <= q <= 20` satisfies `alpha_1 > 1/4` at `(4, 8)`, the lower test run on the restricted matrix of Proposition 3.1; the closest is base `20` missing `0` at `[0.2528608, 0.2531118]`, and the only sets with a lower bound under `0.27` are the digit `0` of the bases `16` to `20`, at `0.2678855`, `0.2635713`, `0.2596685`, `0.2561148` and `0.2528608`, each rerun at its mirror digit, whose transform is the same and whose Lipschitz correction is not, with intersecting brackets. The constant-phase digits, whose cells can empty a row, are all bracketed, base `5` missing `2` at `[0.4366508, 0.4496785]` and base `19` missing `9` at `[0.2732272, 0.2735182]`. So `21` is the least base carrying a one-missing-digit set below `1/4`; Fact 6.9 certifies `106` of the `108` sets a second time by a route that builds no window matrix. Two cross-checks come from the pair route of Section 7: at base `10` its threshold `beta` is certified above `1/4` at every digit, lower bounds `0.2502716` to `0.2541480`, and `beta <= alpha_1` since the `t = 1` term of the infimum defining `beta` is the exponent of one shift of the grid sum; its region census refutes the bases `3` and `4` and the digits `0` and `1` of base `5`, where `alpha/2 > 1/4` makes every refutation force `alpha_1 > 1/4`. Generators: `lab/py/digit-transform-norms`, verb `floor`; `lab/py/mobius-region`, verbs `threshold` and `criterion`.

**Fact 6.3 (the first family).** All `17` distinct sets of base `34` satisfy `alpha_1 < 0.2493701` at `(4, 8)`, the worst at the digit `16` with `[0.2493107, 0.2493701]` and the best at the digit `0` with `[0.2246400, 0.2247052]`. Every base `21 <= q <= 33` carries a digit with `alpha_1 > 1/4` at `(4, 8)`, the digit of largest upper bound at three window digits, the closest being base `33` missing `15` at `[0.2506145, 0.2506783]` and the next base `32` missing `10` at `[0.2528988, 0.2529666]`. Generators: `lab/py/digit-transform-norms`, verbs `least` and `floor`.
...
**Corollary 6.5 (the family floor).** Every base `q >= 34` satisfies `alpha_1(q, a_0) < 1/4` at every excluded digit, and `34` is the least base at which every digit clears: every base below it has a digit above the bar.

**Proof.** Facts 6.3 and 6.4 and Theorem 4.7 from `125` for the first claim; Fact 6.2 below `21` and the witnesses of Fact 6.3 from `21` to `33` for the second. □
...
| `21` | `[0.2498658, 0.2500871]` | `[0.2664296, 0.2666524]` |
...
| `33` | `[0.2260074, 0.2260778]` | `[0.2401948, 0.2404222]` |
| `34` | `[0.2246400, 0.2247052]` | `[0.2493107, 0.2493701]` |

**Table 1.** Certified brackets at `(4, 8)` at the end digit and the middle digit of ten bases. At odd `q` the middle digit is the constant-phase digit, where `(q-1)|hat F(t)| = |D_q(t) - 1|` has real zeros and rows of `M^-` empty, and its lower bound is the restricted test's. The middle digit is not the dearest: base `20` missing `6` reads `[0.2831944, 0.2834134]` and base `26` missing `8` reads `[0.2652263, 0.2653395]`, each above its middle bracket. Generator: `lab/py/digit-transform-norms`, verbs `least` and `floor`.
...
**Fact 6.8 (the floor of the minorant).** `D_q` is real, so `|D_q(t) - e(ct)| >= ||D_q(t)| - 1|` at every phase and `ell_q = ||D_q| - 1|/(q-1)` is a minorant of `|hat F|` at every excluded digit. Through `M^-` at `(4, 8)` it certifies `alpha_1 > 1/4` at every digit at once for every `3 <= q <= 8`, the lowest `0.2522652` at `q = 8`; through `M^+` it puts the `l^1` exponent of the level products of `ell_q` under `1/4` at every `9 <= q <= 20`, `0.2463803` at `9` and `0.2041911` at `20`, so no bound through this minorant reaches the bar there, and Fact 6.2 stays a census of one certificate per set. Generator: `lab/py/digit-transform-norms`, verb `blind`.

**Fact 6.9 (the shift floor).** By the display in the proof of Lemma 2.6, `Sigma_(N+M)(x) >= Sigma_N(q^M x) min_y Sigma_M(y)`, so `min Sigma_N` is supermultiplicative and `alpha_1 >= (1/N) log_q min_x Sigma_N(x)` at every `N`. In interval arithmetic over one period, each factor at digit `j` lowered by `q^j h/2` times the Lipschitz constant `(2 pi/k) sum_(a in F) |a - (q-1)/2|`, `h` the spacing of the shifts, `min_x Sigma_1 > q^(1/4)` at every digit of every `3 <= q <= 15`, and `min_x Sigma_2 > q^(1/2)` at every digit of `16 <= q <= 18` and at every digit of `19` and `20` except the end digit, the tightest being base `18` missing `0` at `alpha_1 > 0.2501422`. One digit cannot go further: `Sigma_1(0) = 2` exactly, and near the grid `Sigma_1(x) = 2 + (pi^2 C/(q-1)) (qx)^2 + O(x^4)` with `C > 0` exactly at the interior digits other than the constant-phase digit and `C < 0` at the end and constant-phase digits, which therefore dip below `2`, the proof and `C` being on [the coprimality page](../notes/coprime.md). Generator: `lab/py/digit-transform-norms`, verbs `shift` and `grid1`.
...
Four things. First, Theorem 5.1 is conditional and Proposition 5.3 says the same expansion returns nothing without the hypothesis; the saving at base `21` is more than `2.29 * 10^(-5)` in the exponent, and the Mobius census on [the Mobius page](../notes/mobius.md) neither confirms nor threatens it. Second, the unconditional route is a program:
...
The route follows the proof of Maynard (2019) with `mu` in place of the primes and is laid out on the Mobius page: the major arcs for `mu` at base-smooth moduli, free of exceptional zeros since every real primitive character of base-smooth modulus has conductor dividing `8 rad(q)`, and the level of distribution on an initial segment are proved there, the bilinear half never sees the coefficients, the region asked for is `alpha_1 < alpha/2` and `beta <= min(1/4, (2/5)(1 - alpha_1))`, of which `alpha_1 < 1/4` is the `t = 1` proxy, and owed is the level of distribution at moduli sharing a factor with the base, which is why `gcd(n, q) = 1` stays in the statement. The gate `k >= q^(3/4)`, the Parseval floor `beta >= 1 - alpha` against `beta <= 1/4`, is met by every one-missing-digit set of base `q >= 4`, the conclusion would save a power of `log x` only, and the route is refuted at base `10` at every digit, where `beta > 1/4` is certified. Third, the floor has no digit-blind proof: the minorant of Fact 6.8 dies at base `9`, and the shift floor of Fact 6.9 is itself one certified inequality per set, which leaves the end digits of `19` and `20` to the window census. Fourth, the uniform bound is never the sharper number, Fact 6.7.

## Open problems

Four things. The gap between `75` and `125`: the largest loss in Section 4 is the peel, and treating the two-run terms directly should move `125` toward `75`. A proof of the floor: if `min_x Sigma_1 = 2` at every interior digit other than the constant-phase digit, as a float scan to base `40` says, then `alpha_1 >= log_q 2 > 1/4` there for `q <= 15` by one inequality; the end and constant-phase digits dip below `2`, and past `15` any such proof needs two digits of the phase. The exponent itself: every statement is a bracket, and whether `alpha_1` is ever rational or algebraic is untouched. And whether the end digit is always the cheapest is not claimed; the middle digit is not the dearest (Table 1), and where the dearest digit sits is not claimed either.

## Reproducibility

Three studies print every number of Sections 4 and 6, each run from the repository root with one verb that asserts its self-checks, each derived apart from the sweep it checks. `uv run python research/lab/py/digit-uniform-bound/ubound.py check` in under half a minute covers the domination, the run identity, the peel, the Lebesgue scan, the threshold with its certificate and the consistency rows; its verbs `threshold`, `lebesgue`, `least`, `window` and `compare`, each under ten seconds, print Facts 4.8 and 6.7. `uv run python research/lab/py/digit-transform-norms/norms.py floor` in `65` seconds prints Fact 6.2 and the witnesses of Fact 6.3, `blind` in `3` seconds Fact 6.8, `shift` in `15` seconds and `grid1` in one second Fact 6.9, `least` in `93` seconds base `34` of Fact 6.3, the shorter windows of Facts 6.1 and 6.6 and Table 1, `six` in `200` seconds Fact 6.6 and the six-digit bracket of Fact 6.1, `family` in `151` seconds Fact 6.4. `uv run python research/lab/py/mobius-region/mobius_region.py params 21 123456789abcdefghijk` in `12` seconds prints the second bracket of Fact 6.1, `threshold` in `41` seconds the base-`10` refutation, `criterion` in `155` seconds the census of `49` designs. The figure is `bash scripts/figures.sh paper-first-base-below-a-quarter`, under three seconds a theme.
