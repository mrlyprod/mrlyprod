...
**Non-Rajchman at every weight (Proved).** `hat mu(base t) = P_w(t) hat mu(t)` with `P_w(t) = sum_f w_f e(-f . t)`, and `P_w = 1` at every integer `t` because `sum_f w_f = 1`, so `hat mu(base^m t) = hat mu(t)` on `Z^dim` and the Fourier dimension is zero whatever the weights. Weights do not rescue the Mobius door ([mobius](mobius.md)).
