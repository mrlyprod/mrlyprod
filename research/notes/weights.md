...
**Non-[Rajchman](/wiki/rajchman-measure/) at every weight (Proved).** `hat mu(base t) = P_w(t) hat mu(t)` with `P_w(t) = sum_f w_f e(-f . t)` - the digit transform `hat F` of [mobius](mobius.md), weighted, normalised and read at `-t`, written `P_w` on this page - and `P_w = 1` at every integer `t` because `sum_f w_f = 1`, so `hat mu(base^m t) = hat mu(t)` on `Z^dim` and the Fourier dimension is zero whatever the weights. Weights do not rescue the Mobius door ([mobius](mobius.md)).
