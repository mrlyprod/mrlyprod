...
lead: The moire of the stacked diagonal slices: the exact cut-ink laws, the doubling sign law, the quarter-line bands, and the ghost star, whose cell-frame decay is a closed form at every band width, `-1/4` at the arm and `-1/8` in the limit, every layer-pair constant an exact rational, and no frame-free coefficient.
figure: research-hexagon
slug: hexagon
---
...
One generator prints every number on this page, `lab/rs/hexagon-moire`, which runs in under a minute and whose sections are the sections below; a number it does not print is not on this page, and the one place a shelf paper is the source instead says so. [The volume demo](../../site/demos/volume/) stacks a cube design into a solid field and cuts it on any plane, the central diagonal cut being this hexagon, and [the slices demo](../../site/demos/slices/) draws one layer triangle by triangle.
