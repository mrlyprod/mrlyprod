...
The generator is `lab/rs/walk-dimension`, which also draws the figure. The [race demo](../../site/demos/race/) runs the experiment live: two base-3 designs of the same fill, random walkers on both at once.
...
Codes 255 and 495 are one symmetry class - the base-3 group wraps residues, so the carpet (centre hole) and the corner-hole design sit in one orbit, as [the bijection page](bijection.md)'s census counts them. Their truncations walk differently: 2.17 to 2.19 against 2.10 to 2.12, stable across two level pairs in both readings. The residue rotation is a symmetry of the rule, not of the drawn pattern, and the walker feels the difference the fixed-side caveat of [the core](core.md) has always recorded for fill counts. (**Verified**, `lab/rs/walk-dimension`.)
