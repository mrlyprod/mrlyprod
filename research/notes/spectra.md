...
- **[A299916](https://oeis.org/A299916) counts holes, not tiles.** Its name is `a(n) = A299914(2n+1)`, offset 0, terms `1, 6, 42, 306, 2250, 16578, 122202`, signature `(9,-12)`; the [Menger](/wiki/menger-sponge/) reading is a comment on the entry, not the definition. That the two are the same object is **Proved**, by the hexagram bijection in [cuts](cuts.md), and the index shift is load-bearing: the mesh-triangle census is `A299916(n+1)`.
...
- **The matrices. Verified** by `lab/py/odd-base-slice-grammar`. `[[7,3],[30,4]]` at base 5, `[[30,3],[24,12]]` at 7, `[[19,9],[96,9]]` at 9, all non-negative, reproducing cell counts level after level. Counts obeying a fixed `2x2` map is weaker than a substitution acting on tiles, and only base 3 has the latter from a source.
