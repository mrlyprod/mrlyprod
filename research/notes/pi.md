...
Every claim below is tagged. **Proved** means proved or classical; **Verified** means recomputed here and reported as measured; **Conjecture** means supported and open. The [Farey demo](../../site/demos/farey/) draws the stack this page counts, with `phi(n)` new nodes at each scale and the primes at maximal novelty.
...
recomputed to `0.785398262` at two million factors against `pi/4 = 0.785398163`; the identity itself is Wallis, **Proved**, and the truncated product is generated by `mrlynum::sieve::ratio` on the odd word of two million letters, pinned at nine digits by `the_plane_ratio_at_two_million_factors_rounds_to_nine_digits`, so that figure is **Verified**. The [Wallis sieve demo](../../site/demos/wallis/) draws the sieve level by level and walks the same product, and the cube analogue's limit `0.948815486` is **Proved** in closed form there. A fixed-ratio carpet has one ratio, not a product of changing ones. That is the whole reason a hunt inside one carpet fails, and it is worth logging so nobody chases it.
