...
Every claim carries a tag. **Proved** means a proof is given or restated here; **Verified** means recomputed from scratch by a crate test or a lab study; **Conjecture** means neither. The generators are `mrlymath::shape` in `../crates` - `classify`, `regions`, `crop`, `refine`, `census`, each pinned by tests named for their claims, including an independent `2^D`-corner oracle - and `lab/crop-counts`, the one pass that prints every number below.
