...
- `check` runs three checks per cell to `10^13`: every element of `F(B, A)` below the height passes a digit test in each original base; the joint count from enumerating the lowest-dimension original design and filtering it by the others equals the count of `F(B, A)`, which with the first check gives set equality; and the joint count below `B^L` equals `(card A)^L` at every `L` with `B^L <= 10^13`. Runtime `16` s, one core, peak resident `252` MB run alone and `253` MB on the three-verb run.
...
- `bases.md:124` the three checks to `10^13` and the counts `16384`, `1048576`, `6561`, `4096` at `L = 7, 10, 4, 3`
