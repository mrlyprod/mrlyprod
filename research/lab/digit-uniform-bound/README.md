...
- The cost of uniformity grows with `m` because the phase thrown away is `m` characters rather than one: the majorant loses `m/(base-m)` where the truth loses the interference between them, and the two-digit numeric floor is far below `650` (`lab/digit-transform-norms`, verb `pairfail`).
