...
- The census of every width-`k` rule at base 2 for `(D, k)` in `{(1,1), (1,2), (1,3), (1,4), (2,1), (2,2)}`: class counts under two groups, Perron roots as exact algebraic numbers, and the memory number `kappa`.

## THE DIAL

- The alphabet is the `2^D` digit vectors `d in {0,1}^D`, each read as the corner integer `c = sum_i d[i] 2^i` with axis `i` on bit `i`.
- A width-`k` rule is a set `W` of windows `(d_1, ..., d_k)`; the window integer is `w = sum_(j=1..k) c_j 2^(D (k - j))`, the first digit most significant.
- The rule code has bit `w` set iff window `w` is allowed, so a rule at `(D, k)` is a code in `[0, 2^(2^(kD)))`: `4, 16, 256, 65536` rules at `D = 1` and `k = 1..4`, `16` and `65536` at `D = 2` and `k = 1, 2`.
- A word `d_1 d_2 ... d_L` is read coarsest digit first and is accepted iff every window of `k` consecutive digits is allowed; every word of length `L < k` is accepted.
- `N_W(L)` is the number of accepted words of length `L`, and `#W` the number of allowed windows.
- At `k = 1` the code is a bang code and the accepted words of length `L` are the level-`L` cells of the design of that code, so the memory dial at `k = 1` is the plain design. **Verified** (memory-census, the `(D,1)` rows of `census.csv`).
- `mrlymath::bang::universe::corners(D)` emits the corner vector row first and `corner_index` folds it most significant first, so a crate design's corner integer at `D = 2` is `c = x + 2y`, bit `0` the column: the same map this census uses, and no code label moves. **Verified** (the `mrlydemo` test `width_one_is_the_plane_design_cell_for_cell` pins codes `11` and `13`, which the axis swap exchanges and whose designs differ).

## THE TRANSFER MATRIX

- The states are the `2^(D(k-1))` windows of width `k - 1`; `A_W[s][t] = 1` iff `s` and `t` overlap in `k - 2` digits and the `k`-window they form is allowed. At `k = 1` there is one state and `A_W = [#W]`.
- `N_W(L) = 1^T A_W^(L-k+1) 1` for `L >= k - 1`. **Proved** (a word of length `L` is a path of `L - k + 1` steps: its width-`(k-1)` windows in order are the states, each consecutive pair forms one width-`k` window, and every such path is an accepted word).
...
- `lim_L N_W(L)^(1/L) = rho(W)` whenever `rho(W) > 0`. **Proved** (`max_(s,t) (A^n)[s][t] <= 1^T A^n 1 <= S^2 max_(s,t) (A^n)[s][t]` with `S` the number of states, and the spectral radius is the limit of the `n`-th root of the matrix norm).
- `log_2 rho(W)` is the growth exponent of `N_W(L)`, which is what this study measures. That it is also the Hausdorff dimension per axis of the accepted set is **Conjecture** here, not proved and not cited.
...
- `kappa(W) = 0` when `W = F^k` is a product with `F` non-empty, the memoryless case. **Proved** (every word over `F` is accepted, so `N_W(L) = #F^L`, `rho = #F >= 1` and `#W = #F^k`). At `F = {}` the rule is dead and `kappa` is not defined.
...
- At `k = 1` every live rule is a product, `kappa` is identically `0`, and the maximum is attained on every live class at once, the full rule at `rho = 2^D` included. **Verified** (memory-census, the `(D,1)` rows).
...
- The window budget of zero entropy, the largest `#W` a live class with `rho = 1` carries, is `1, 3, 6, 13` at `D = 1` and `k = 1..4` and `1, 10` at `D = 2`: a rule may allow that many windows and still accept subexponentially many words, and code `11` at `(1,2)`, which allows `00, 01, 11`, accepts every `0^a 1^b`, `N_W(L) = L + 1`. **Verified** (memory-census, `census.csv` column `rho1_windows`).

## THE GROUP

- `G_(D,k)` is `B_D`, the signed permutations of the `D` axes, acting by the *same* element on every digit of a window, together with window reversal `(d_1, ..., d_k) -> (d_k, ..., d_1)`.
- Every element preserves `N_W(L)` for every `L`. **Proved** (a diagonal `B_D` element relabels the digit alphabet by a bijection, which relabels the states and conjugates `A_W` by a permutation matrix; reversal sends a word to its reverse, a bijection of accepted words, and transposes `A_W`).
- Reversal therefore fixes the characteristic polynomial as well, since `A` and `A^T` share it.
- A width-`k` rule in dimension `D` is the same object as a subset of the `kD`-cube: the same censuses of the `kD`-cube counted with a smaller group. `#G_(D,k) = 2^(D+1) D!` for `k >= 2` against `#B_(kD) = 2^(kD) (kD)!`; at `k = 1` reversal is trivial and `#G_(D,1) = 2^D D!`.
- The distinct window permutations are `2, 4, 4, 4` at `D = 1` and `k = 1..4` (reversal is trivial at `k = 1`) and `8, 16` at `D = 2` and `k = 1, 2`. **Verified** (memory-census, `census.csv` columns `group_G`, `group_B`).
...
- One row per `(D, k)` in `census.csv`, one row per distinct Perron minimal polynomial in `classes.csv`.
- Classes under `G_(D,k)`: `3, 9, 88, 16960` at `D = 1` and `k = 1..4`; `6, 4660` at `D = 2` and `k = 1, 2`. **Verified** (memory-census, orbit walk, agreeing with an independent Burnside average on every row).
- Classes under diagonal `B_D` alone, no reversal: `3, 10, 136, 32896` at `D = 1`; `6, 8548` at `D = 2`. **Verified** (memory-census, same two verbs).
- At `k = 1` the two groups agree and the counts are A000616 at `D`: `3` at `D = 1` and `6` at `D = 2`, the design census reproduced. **Verified** (memory-census, the `(D,1)` rows).
...
- At `(2,2)` the same four occur on codes `327, 19, 323, 326` with `121, 588, 54, 48` classes, so the named roots are not a `D = 1` accident. **Verified** (memory-census, `classes.csv`).
...
- So every root of a live rule is either `1` or a radical of a strict Perron number in `(1, 2^D]`; the dead rules carry `rho = 0`, on `1, 2, 13, 2093` classes at `D = 1` and `1, 53` at `D = 2`, and `0` is a radical of nothing. Witnesses: `x^2 - 2`, `x^3 - 2` and `x^4 - 2` at `(1,4)`, and `x^4 - x^2 - 1` and `x^6 - x^3 - 1`, the golden ratio's square and cube roots. **Verified** (memory-census, `classes.csv`).

## OEIS

- `3, 10, 136, 32896, 2147516416`, the `D = 1` count with no reversal, greps three hits in the local dump: A055708, A056006, A191363. All three are lists of integers with a `sigma` property and the agreement is a coincidence of the closed form `2^(m-1)(2^m + 1)` at `m = 2^(k-1)`; none is this census. **Verified** (grep `,3,10,136,32896` in the local dump, then the name of each hit read at source).
- `3, 9, 88, 16960` (classes under `G_(1,k)`), `6, 4660` (under `G_(2,k)`), `3, 4, 10, 177` (distinct Perron minimal polynomials at `D = 1`) and `3, 6, 23, 431` (distinct characteristic polynomials at `D = 1`) each grep to zero hits. **Verified** (grep `,3,9,88,16960`, `,6,4660,`, `,3,4,10,177,`, `,3,6,23,431,` in the local dump).
