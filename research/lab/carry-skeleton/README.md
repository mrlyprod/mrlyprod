...
- `CARGO_BUILD_JOBS=4 cargo run --release --manifest-path research/lab/Cargo.toml -p carry-skeleton` from the repo root. `7.53s` on one thread, `7.43s` of it the depth sampling. Prints only, writes nothing.
- `CARGO_BUILD_JOBS=4 cargo test --manifest-path research/lab/Cargo.toml -p carry-skeleton`, `12` tests, under a second after the build.
