...
- The spin mass `M(r)` is read twice: exactly, as the integer shell histogram of filled cells about a fixed point, and through `mrlyrs::math::spin::profile` with `mass_within`, the two agreeing to `1.7e-6` on the total and to `0.5%` on partial radii. The hole about the raster centre is measured on cell rectangles in exact integer arithmetic, never on cell centres, which would return `hole + 1/2` whatever the hole.
...
- The spin spectrum is `mrlyrs::math::spin::harmonics` at levels 1 and 2 over all 511 nonempty codes, orders `m = 0..12`, compared against the 101 orbits of the square group.
...
- `mrlyrs::math::spin::mass_within` and the crate test `the_spin_mass_scales_by_the_fill_about_a_filled_corner`; the window regression test `the_window_counts_whole_periods_at_every_level`.
