...
- Runs the mini preset of `atlas` and reads every settled frame as a design: the fates, the per-frame readings, and the Kronecker block test of [code-factorisation](../code-factorisation/README.md) on every settled frame and on every run's heatmap level set.
...
- `CARGO_BUILD_JOBS=4 cargo run --release -p life-atlas`
...
- `atlas::readings::block_split`, `atlas::readings::first_negative_lobe`, `atlas::run::heat_frame`, `atlas::run::census`, the crate paths behind every number.
