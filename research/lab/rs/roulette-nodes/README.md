...
- The counter is `mrlylab::roulette::nodes` over `mrlyrs::num::spirograph::trace`: every pair of polyline segments is tested for a proper crossing by orientation signs on a grid of buckets, each pair judged in the one bucket the two bounding boxes first share, so no crossing is counted twice and no pair is missed.
