...
- Coincident pencils are dropped before counting by `mrlyrs::math::roulette::spread`: on a circle the seats fall into classes under the rotation group of order `gcd(b, 4)`, and on a line or a polygon every distinct seat draws its own curve, two seats of one radius on a line drawing translates of one shape and never one curve.
...
- `CARGO_BUILD_JOBS=4 cargo run --release -p roulette-nodes`
