...
- The residues at `base = 3` reproduce the certified interval enclosures of `lab/burnol-residue` digit for digit on both `F = {0,1}` and `F = {0,2}`, by a route that shares no code with them.
