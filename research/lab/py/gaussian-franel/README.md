...
- The kernel is the gcd matrix `N(gcd(a,b))^2 / (N(a) N(b))`, the Gaussian twin of the `gcd(m,n)^2/(mn)` that is the layer Gram matrix of the parity carpet stack on [the stack page](../../../notes/stack.md). The same Smith kernel carries the moire correlation law and the Franel identity.
...
- `franel_form` on the rational data at `Q = 125` to `8000`: `S2 * Q = (C(Q) - 1) Q / (12 Phi(Q))` reads `0.5395, 0.5848, 0.6241, 0.6387, 0.6560, 0.6538, 0.6564`, regenerating the Farey discrepancy table of [the Farey page](../../../notes/farey.md) with no Farey enumeration anywhere.
...
- `uv run python research/lab/py/gaussian-franel/gaussian_franel.py`
