...
function thumb(design, dimension) {
  if (dimension === 2) return <Grid grid={m.two_grid(design.code, 3, 2, 0, 2)} on={tint(design)} className="" />;
  try {
    return <Markup svg={m.hex_svg(design.code, 3, 1, 2, 'iso', 6)} />;
  } catch {
    return <Markup svg='<svg viewBox="0 0 1 1"></svg>' />;
  }
}

function grown(design, dimension, level) {
  if (dimension === 2) return <Grid grid={m.two_grid(design.code, 3, level, 0, 2)} on={tint(design)} style={{ maxWidth: 486 }} />;
  try {
    return <Markup svg={m.hex_svg(design.code, 3, level, 2, 'iso', level === 3 ? 2 : 8)} />;
  } catch (error) {
  ...
  return (
    <Page crumb="universe" title="The universe of designs"
      ...
      <Row>
        ...
        <span className="num" style={{ color: 'var(--dim)' }}>{world.distinct} designs from {world.total} codes</span>
      </Row>
      <div className="cards">
        {cards.map(([one, art]) => (
          <div key={one.code} className={one.code === design.code ? 'card on' : 'card'} onClick={() => setCode(one.code)}>
            {art}
