...
function App() {
  ...
  const art = (block) => {
    if (dim === 2) return <Grid grid={block.grid} on={ink.gold} />;
    if (dim === 6) return <Markup svg={block.svg} />;
    return null;
  };
  ...
  return (
    <Page crumb="tower" title="The tower"
      ...
      <Row>
        ...
        <span className="badge dim">{name ? `${name} · ` : ''}{spell(codes, numbers, bases)}</span>
      </Row>
      <Stage hidden={dim !== 3} deps={[tower]} onStage={onStage} />
      <div className="tower">
      ...
      <Sketch draw={drawChart} deps={[tower, dim]} className="bars" />
      <Stats>
