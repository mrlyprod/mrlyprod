import { ready, $, ink, paint, say, out } from '../lib/mrly.js';
import { seeds } from '../lib/select.js';

const m = await ready();
const W = 96, H = 96, BUDGET = 8;
const s = seeds();
let grid, generation = 0, running = false;
...
function reseed() {
  ...
  $('verdict').textContent = '';
  if (choice === 'noise') {
    grid = m.life_noise(W, H, +$('density').value, s.get() || 1);
...
$('density').oninput = () => { out('density', $('density').value); reseed(); };
$('sheet').onclick = (event) => {
