...
let dimension = new URLSearchParams(location.search).get('d') === '3' ? 3 : 2, chosen = null;
...
function grow() {
  ...
  $('open').href = `./sponge?code=${design.code}&number=3&base=2`;
  $('orbit').textContent = design.orbit;
  $('degree').textContent = design.degree;
  $('anf').textContent = design.anf;
  $('fills').textContent = `${m.fills(design.code, 3, dimension, level, 2)} of ${3 ** (dimension * level)}`;
