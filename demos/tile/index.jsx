...
function App() {
  ...
  return (
    <Page crumb="tile" title="The tile"
      ...
      <Row>
        ...
        {q.dim === 6 && <Pick label="projection" value={q.proj} options={PROJECTIONS} onChange={(v) => set({ proj: v })} />}
        {q.dim === 6 && <Check label="keep the overhang" checked={q.overhang} onChange={(v) => set({ overhang: v })} />}
        {q.dim === 3 && <Check label="spin" checked={spin} onChange={turn} />}
      </Row>
      {q.dim === 2 && data.grid && <Grid grid={data.grid} on={ink.gold} />}
      <Stage hidden={q.dim !== 3} deps={[data]} onStage={onStage} />
      {q.dim === 6 && <Markup svg={data.art ?? ''} />}
      <Stats>
