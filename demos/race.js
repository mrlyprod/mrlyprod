import { ready, $, ink, rgb } from './mrly.js';

const m = await ready();
const NUMBER = 3, LEVEL = 4, BASE = 3, WALKERS = 300, TARGET = 20, TICKS = 12, SCALE = 6;
...
class Side {
  constructor(code, id, color) {
    ...
    $(`fill-${id}`).textContent = `${m.fills(code, NUMBER, 2, LEVEL, BASE)} of ${Number(m.fills(code, NUMBER, 2, LEVEL, BASE)) + Number(m.voids(code, NUMBER, 2, LEVEL, BASE))}`;
  }
  reset(seed) {
...
let seed = 1, running = false, done = false;
