...
function App() {
  ...
  const letters = () => {
    ...
    return (
      <>
        <Sketch draw={ribbon([top.map(hue), low.map(hue)], top.length)} deps={[top, low, parity]} />
        <div className="stats">
  ...
  const runs = () => {
    ...
    return (
      <>
        <Sketch draw={ribbon([read.digits.map(hue), lengths.map((run) => (run === 1 ? ink.gold : ink.pink))], read.length)} deps={[read]} />
        <div className="stats">
        ...
        <Sketch draw={ribbon([read.boundary.map(hue), read.doubling.map(hue)], read.boundary.length)} deps={[read]} />
        <div className="stats">
  ...
  const card = (row, grid) => (
    <div className="panel" key={row.name}>
      ...
      <div className="stats">
        {verdict(row)}
        {row.folds ? <span>base tile <Signs grid={tile(row.tile)} className="" style={{ width: 22, height: 22, borderRadius: 4, verticalAlign: 'middle', imageRendering: 'pixelated' }} /></span> : null}
  ...
  const pane = (label, grid, on, note) => (
    <div key={label}>
      {pick.fold === 'sign' ? <Signs grid={grid} /> : <Grid grid={grid} on={on} />}
