...
import { mount, Page, Row, Pick, Slider, Check, Text, Btn, Stats, Stat, Note } from '../lib/app.jsx';
...
function View({ row, view, dig, label }) {
  if (view === 'ratios') return <Ratios values={row.ratios} start={row.start} label={`${label}, each term against the one before`} />;
  if (view === 'differences') return <Differences rows={row.differences} label={`${label}, each row the differences of the row above`} />;
  if (view === 'digits') return <Digits terms={row.terms} start={row.start} base={dig} label={label} />;
  return <Pins terms={row.terms} start={row.start} log={view === 'log'} label={label} />;
}
    ...
    <Page crumb="plot" title="Every sequence the designs write, drawn"
      ...
      <div className={pick.on ? 'arena' : undefined}>
        <div className="panel">
          ...
          {row && <View row={row} view={pick.view} dig={pick.dig} label={label} />}
        ...
        {pick.on && (
          <div className="panel">
            ...
            {blend.row && <View row={blend.row} view={pick.view} dig={pick.dig} label={`${PAIRED.includes(pick.op) ? 'both sequences' : 'the first sequence'}, index from 0`} />}
