...
function App() {
  ...
  return (
    <Page crumb="crop" title="The crop"
      ...
      <div className="arena" style={{ gridTemplateColumns: '3fr 2fr' }}>
        <div className="panel">
          ...
          {data.grid && <Grid grid={data.grid} on={ink.gold} />}
          <Markup style={ART} hidden={!data.art} svg={data.art ?? ''} />
          <Stage hidden={!data.solid} deps={[data]} onStage={(st) => {
            live.current = st;
        ...
        <div className="panel">
          ...
          <Sketch className="bars" draw={alongRadius} deps={[data]} onSeek={(frac) => set({ radius: Math.min(RMAX, Math.max(0, Math.round(frac * RDEN))) })} />
          <h2>Along the level <span>kept fills, log scale</span></h2>
          <Sketch className="bars" draw={alongLevel} deps={[data]} />
        </div>
      </div>
      <Stats>
