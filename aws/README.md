...
- `net.ts` is the `mrlynet-site` Lambda: a push wakes it with the sha, it builds the site at that sha and pushes what changed.
...
- The Lambda never runs cargo: `scripts/wasm.sh` only builds `pkg/`; the console outside this repo rebuilds and uploads it on every push and `sites/net/pkg.lock` names the prefix.
