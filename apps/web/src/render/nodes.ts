...
function scaled(node: Extract<Node, { kind: "Range" }>, raw: string): string {
  const scale = node.scale ?? 1
  if (scale <= 1) return raw
  return String(Number((Number(raw) / scale).toFixed(String(scale).length - 1)))
}
...
export function create(node: Node, send: Send): Held {
  switch (node.kind) {
    ...
    case "Range": {
      ...
      input.addEventListener("input", () => {
        value.textContent = scaled(held(el) as Extract<Node, { kind: "Range" }>, input.value)
      })
...
export function patch(el: Held, node: Node, send: Send): void {
  ...
  switch (node.kind) {
    ...
    case "Canvas": {
      ...
      surface.style.touchAction =
        node.tap !== undefined || node.drag !== undefined || node.turn !== undefined || node.pan !== undefined || node.zoom !== undefined
          ? "none"
          : ""
      el.classList.toggle("crisp", node.cells !== undefined || node.tris !== undefined)
    ...
    case "Range": {
      ...
      value.textContent = scaled(node, input.value)
      break
    }
    case "Overlay":
