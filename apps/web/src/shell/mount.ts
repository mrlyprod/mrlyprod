...
import type { Call, KeySet, Manifest, Node, Observation, Send } from "../types.ts"
...
export function mount(root: HTMLElement, send: Send, routes: Router, apps: Manifest[], goose: (seed: number) => Observation): Ui {
  ...
  routes.on("splash.", call => {
    ...
    if (call.verb === "splash.on") shoo()
    else if (arcade(topo)) lure()
  })

  const keys: Record<string, KeySet> = {}
  for (const app of apps) {
    if (app.keys !== undefined) keys[app.route] = app.keys
  }
  ...
  const start = make("button", "mrly-box link-box start")
  start.textContent = "start"
  start.hidden = true
  start.addEventListener("click", () => wake())
  ...
  root.replaceChildren(bar, title, column, foot, sheet, toast, start, splash)
  ...
  let idle = false
  let strut: ReturnType<typeof setInterval> | undefined
  const arcade = (app: string) => {
    const category = apps.find(a => a.route === app)?.category
  ...
  const wake = () => {
    shoo()
    const app = current?.view?.app
    if (app !== undefined) send({ verb: `${app}.reset`, args: {} })
  }
  ...
  const render = (obs: Observation) => {
    ...
    if (next !== topo) {
      shoo()
      if (arcade(next) && !root.classList.contains("splashed")) lure()
    }
    topo = next
    repaint(obs)
  }
