  ...
  return (
    <stack key="waves">
      ...
      <card key="controls">
        ...
        <button key="accent" call={pickOpen("colors", "waves", "accent", s.settings.accent)}>{`accent · ${s.settings.accent}`}</button>
        <button key="anti" call={pickOpen("colors", "waves", "anti", s.settings.anti)}>{`anti · ${s.settings.anti}`}</button>
        <Shot />
      </card>
      <card key="physics">
