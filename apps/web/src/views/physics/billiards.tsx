...
export function billiards(state: unknown, _send: Send): Node {
  ...
  return (
    <stack key="billiards">
      ...
      <card key="physics">
        <range key="speed" value={s.settings.speed} min={50} max={400} scale={100} call={turn("speed")} arg="value" step={10} label="speed" />
        <range key="trail" value={s.settings.trail} min={2} max={100} scale={100} call={turn("trail")} arg="value" step={2} label="trail" />
        <range key="size" value={s.settings.size} min={100} max={400} scale={100} call={turn("size")} arg="value" step={50} label="size" />
